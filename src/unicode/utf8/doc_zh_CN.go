...
// DecodeLastRune 解包 p 中的最后一个UTF-8编码，并返回该符文及其字节宽度。 若此
// 编码无效，它就会返回 (RuneError, 1)，二者对于正确、非空的UTF-8来说均为不可能
// 的值。 若一个编码为错误的UTF-8值，或该符文的编码超出范围，或不是该值可能的最
// 短UTF-8编码， 那么它就是无效的。除此之外，并不进行其它的验证。
func DecodeLastRune(p []byte) (r rune, size int)

// DecodeLastRuneInString is like DecodeLastRune but its input is a string. If s
// is empty it returns (RuneError, 0). Otherwise, if the encoding is invalid, it
...
// DecodeLastRuneInString 类似于 DecodeLastRune，但其输入为字符串。 若此编码无
// 效，它就会返回 (RuneError, 1)，二者对于正确、非空的UTF-8来说均为不可能的值。
// 若一个编码为错误的UTF-8值，或该符文的编码超出范围，或不是该值可能的最短UTF-8
// 编码， 那么它就是无效的。除此之外，并不进行其它的验证。
...
// encoding is invalid, it returns (RuneError, 1). Both are impossible results
...
// DecodeRune 解包 p 中的第一个UTF-8编码，并返回该符文及其字节宽度。 若此编码无
// 效，它就会返回 (RuneError, 1)，二者对于正确、非空的UTF-8来说均为不可能的值。
// 若一个编码为错误的UTF-8值，或该符文的编码超出范围，或不是该值可能的最短UTF-8
// 编码， 那么它就是无效的。除此之外，并不进行其它的验证。
...
// DecodeRuneInString 类似于 DecodeRune，但其输入为字符串。 若此编码无效，它就会
// 返回 (RuneError, 1)，二者对于正确、非空的UTF-8来说均为不可能的值。 若一个编码
// 为错误的UTF-8值，或该符文的编码超出范围，或不是该值可能的最短UTF-8编码， 那么
// 它就是无效的。除此之外，并不进行其它的验证。
