...
// io 包为I/O原语提供了基础的接口. 它主要包装了这些原语的已有实现，如 os 包中的
// 那些，抽象成函数性的共享公共接口， 加上一些其它相关的原语。
//
// 由于这些接口和原语以不同的实现包装了低级操作，因此除非另行通知， 否则客户不应
// 假定它们对于并行执行是安全的。
package io
...
// EOF 是 Read 在没有更多输入可用时返回的错误。 函数应当只为标志出优雅的输入结束
...
//
// Read 将 len(p) 个字节读取到 p 中。它返回读取的字节数 n（0 <= n <= len(p)） 以
// 及任何遇到的错误。即使 Read 返回的 n < len(p)，它也会在调用过程中使用 p 的全
// 部作为暂存空间。若一些数据可用但不到 len(p) 个字节，Read 会照例返回可用的东
// 西， 而不是等待更多。
//
// 当 Read 在成功读取 n > 0 个字节后遇到一个错误或 EOF 情况，它就会返回读取的字
// 节数。 它会从相同的调用中返回（非nil的）错误或从随后的调用中返回错误（和 n ==
// 0）。 这种一般情况的一个例子就是 Reader 在输入流结束时会返回一个非零的字节
// 数， 可能的返回不是 err == EOF 就是 err == nil。无论如何，下一个 Read 都应当
// 返回 0, EOF。
//
// 调用者应当总在考虑到错误 err 前处理 n > 0 的字节。这样做可以在读取一些字节，
// 以及允许的 EOF 行为后正确地处理I/O错误。
//
// Read 的实现在 len(p) == 0 以外的情况下会阻止返回零字节的计数和 nil 错误， 调
// 用者应将返回 0 和 nil 视作什么也没有发生；特别是它并不表示 EOF。
//
// 实现必须不保留 p。
...
//
// ReadAt 从基本输入源的偏移量 off 处开始，将 len(p) 个字节读取到 p 中。 它返回
// 读取的字节数 n（0 <= n <= len(p)）以及任何遇到的错误。
//
// 当 ReadAt 返回的 n < len(p) 时，它就会返回一个非nil的错误来解释 为什么没有返
// 回更多的字节。在这一点上，ReadAt 比 Read 更严格。
//
// 即使 ReadAt 返回的 n < len(p)，它也会在调用过程中使用 p 的全部作为暂存空间。
// 若一些数据可用但不到 len(p) 字节，ReadAt 就会阻塞直到所有数据都可用或产生一个
// 错误。 在这一点上 ReadAt 不同于 Read。
//
// 若 n = len(p) 个字节在输入源的的结尾处由 ReadAt 返回，ReadAt 不是返回 err ==
// EOF 就是返回 err == nil。
//
// 若 ReadAt 按查找偏移量从输入源读取，ReadAt 应当既不影响基本查找偏移量也不被它
// 所影响。
//
...
//
// ReadFrom 从 r 中读取数据，直到 EOF 或发生错误。其返回值 n 为读取的字节数。 除
// io.EOF 之外，在读取过程中遇到的任何错误也将被返回。
//
...
// 表示相对于文件的起始处，1 表示相对于当前的偏移，而 2 表示相对于其结尾处。
// Seek 返回新的偏移量和一个错误，如果有的话。
//
// 从文件开头向前 Seek 偏移量会产生错误。对任何正数偏移量进行 Seek 都是合法的，
// 但对底层对象的后续 I/O 操作行为则取决于具体实现。
...
// n（0 <= n <= len(p)）以及任何遇到的引起写入提前停止的错误。若 Write 返回的 n
// < len(p)，它就必须返回一个非nil的错误。Write 不能修改此切片的数据，即便它是临
// 时的。
//
// 实现必须不保留 p。
...
// off. It returns the number of bytes written from p (0 <= n <= len(p)) and any
...
//
// 若 WriteAt 按查找偏移量写入到目标中，WriteAt 应当既不影响基本查找偏移量也不被
// 它所影响。
//
...
//
// WriteTo 将数据写入 w 中，直到没有数据可读或发生错误。其返回值 n 为写入的字节
// 数。 在写入过程中遇到的任何错误也将被返回。
//
...
// Copy 将 src 复制到 dst，直到在 src 上到达 EOF 或发生错误。 它返回复制的字节
// 数，如果有的话，还会返回在复制时遇到的第一个错误。
//
// 成功的 Copy 返回 err == nil，而非 err == EOF。由于 Copy 被定义为从 src 读取直
// 到 EOF 为止，因此它不会将来自 Read 的 EOF 当做错误来报告。
//
// 若 src 实现了 WriterTo 接口，其复制操作可通过调用 src.WriteTo(dst) 实现。 否
// 则，若 dst 实现了 ReaderFrom 接口，其复制操作可通过调用 dst.ReadFrom(src) 实
// 现。
...
// MultiWriter 创建一个 Writer，它将其写入复制到所有提供的 writers 中，类似于
// Unix的tee(1)命令。
...
// Pipe 创建同步的内存管道。 它可用于将代码预期的 io.Reader 连接到代码预期的
// io.Writer。 一端的读取匹配另一端的写入，直接在这两端之间复制数据；它没有内部
// 缓存。 它对于并行调用 Read 和 Write 以及其它函数或 Close 来说都是安全的。 一
// 旦等待的I/O结束，Close 就会完成。并行调用 Read 或并行调用 Write 也同样安全：
// 同种类的调用将按顺序进行控制。
...
// error is EOF only if no bytes were read. If an EOF happens after reading
// fewer than min bytes, ReadAtLeast returns ErrUnexpectedEOF. If min is greater
// than the length of buf, ReadAtLeast returns ErrShortBuffer. On return, n >=
// min if and only if err == nil.

// ReadAtLeast 将 r 读取到 buf 中，直到读了最少 min 个字节为止。 它返回复制的字
// 节数，如果读取的字节较少，还会返回一个错误。若没有读取字节， 错误就只是 EOF。
// 如果一个 EOF 发生在读取了少于 min 个字节之后，ReadAtLeast 就会返回
// ErrUnexpectedEOF。若 min 大于 buf 的长度，ReadAtLeast 就会返回
// ErrShortBuffer。 对于返回值，当且仅当 err == nil 时，才有 n >= min。
...
// 如果读取的字节较少，还会返回一个错误。若没有读取字节， 错误就只是 EOF。如果一
// 个 EOF 发生在读取了一些但不是所有的字节后，ReadFull 就会返回
// ErrUnexpectedEOF。 对于返回值，当且仅当 err == nil 时，才有 n == len(buf)。
...
// TeeReader 返回一个 Reader，它将从 r 中读到的东西写入 w 中。 所有经由它处理的
// 从 r 的读取都匹配于对应的对 w 的写入。它没有内部缓存， 即写入必须在读取完成前
// 完成。任何在写入时遇到的错误都将作为读取错误来报告。
...
// Close 关闭读取器；关闭后如果对管道的写入端进行写入操作，就会返回
// ErrClosedPipe 错误。
...
// CloseWithError 关闭读取器；关闭后如果对管道的写入端进行写入操作，就会返回 err
// 错误。
...
// Close 关闭写入器；关闭后如果对管道的读取端进行读取操作，就会返回 EOF 而不返回
// 字节。
...
// CloseWithError 关闭写入器；关闭后如果对管道的读取端进行读取操作，就会返回错误
// err 而不返回任何字节。
//
