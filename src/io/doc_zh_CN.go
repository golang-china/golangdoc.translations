...
// LimitedReader 从 R 读取但将返回的数据量限制为 N 字节。每调用一次 Read 都将更
// 新 N 来反射新的剩余数量。
...
//
// 对负数偏移量进行 Seek 会产生错误。对任何正数偏移量进行 Seek 是合法的，但对底
// 层类型的后续 I/O 操作行为则取决于具体实现。
...
// LimitReader 返回一个 Reader，它从 r 中读取 n 个字节后以 EOF 停止。 其基本实现
// 为 *LimitedReader。
...
// MultiReader 返回一个 Reader，它是输入 readers 提供的的逻辑拼接。 它们按顺序读
// 取。一旦所有的输入返回 EOF，Read 就会返回 EOF。 若任何 readers 返回了非 nil
// 或非 EOF 错误，Read 就会返回该错误。
...
// NewSectionReader 返回一个 SectionReader，它从 r 中的偏移量 off 处读取 n 个字
// 节后以 EOF 停止。
...
// ErrUnexpectedEOF。若 min 大于 buf 的长度，ReadAtLeast 就会返回 ErrShortBuffer
...
// 个 EOF 发生在读取了一些但不是所有的字节后，ReadFull 就会返回 ErrUnexpectedEOF
...
// WriteString 将字符串 s 的内容写入 w 中，它接受一个字节数组。 若 w 已经实现了
// WriteString 方法，就可以直接调用它。
