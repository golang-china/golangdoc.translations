...
// io 包为I/O原语提供了基础的接口. 它主要包装了这些原语的已有实现，如 os
...
// clients of an io.Reader when many calls
// to Read have failed to return any data
// or error, usually the sign of a broken
// io.Reader implementation.
...
// 或发生错误。 它返回复制的字节数，如果有的话，还会返回在复制时遇到的第一个错误。
//
// 成功的 Copy 返回 err == nil，而非 err == EOF。由于
// Copy 被定义为从 src 读取直到 EOF 为止，因此它不会将来自 Read
// 的 EOF 当做错误来报告。
//
// 若 src 实现了 WriterTo 接口，其复制操作可通过调用
// src.WriteTo(dst) 实现。 否则，若 dst 实现了
// ReaderFrom 接口，其复制操作可通过调用
// dst.ReadFrom(src) 实现。
...
// 错误就只是 EOF。如果一个 EOF 发生在读取了少于 min
// 个字节之后，ReadAtLeast 就会返回
// ErrUnexpectedEOF。若 min 大于 buf
// 的长度，ReadAtLeast 就会返回 ErrShortBuffer。
...
// 中。 它返回复制的字节数，如果读取的字节较少，还会返回一个错误。若没有读取字节，
// 错误就只是 EOF。如果一个 EOF
// 发生在读取了一些但不是所有的字节后，ReadFull 就会返回
// ErrUnexpectedEOF。 对于返回值，当且仅当 err == nil
// 时，才有 n == len(buf)。
...
//
// ReadByte 从输入中读取并返回下一个字节。若没有字节可用，就会置为
// err。
...
// ByteScanner 接口将 UnreadByte 方法添加到基本的
// ReadByte 方法。
//
// UnreadByte 使下一次调用 ReadByte 返回的字节与上一次调用
// ReadByte 返回的相同。 调用 UnreadByte 两次而中间没有调用
// ReadByte 的话就会返回错误。
...
// LimitedReader 从 R 读取但将返回的数据量限制为 N
// 字节。每调用一次 Read 都将更新 N 来反射新的剩余数量。
...
// Pipe 创建同步的内存管道。 它可用于将代码预期的 io.Reader
// 连接到代码预期的 io.Writer。
// 一端的读取匹配另一端的写入，直接在这两端之间复制数据；它没有内部缓存。
// 它对于并行调用 Read 和 Write 以及其它函数或 Close
// 来说都是安全的。 一旦等待的I/O结束，Close 就会完成。并行调用 Read
// 或并行调用 Write 也同样安全： 同种类的调用将按顺序进行控制。
...
// Close 关闭写入器；关闭后如果对管道的读取端进行读取操作，就会返回 EOF
// 而不返回字节。
...
// CloseWithError
// 关闭写入器；关闭后如果对管道的读取端进行读取操作，就会返回错误 err
// 而不返回字节。
...
// 若读取端带错误关闭，该错误将作为 err 返回；否则 err 为
// ErrClosedPipe。
...
// ReadWriteCloser 接口组合了基本的 Read、Write 和
// Close 方法。
...
// ReadWriteSeeker 接口组合了基本的 Read、Write 和
// Seek 方法。
...
//
// Read 将 len(p) 个字节读取到 p 中。它返回读取的字节数 n（0
// <= n <= len(p)） 以及任何遇到的错误。即使 Read 返回的 n
// < len(p)，它也会在调用过程中使用 p
...
// 情况，它就会返回读取的字节数。
// 它会从相同的调用中返回（非nil的）错误或从随后的调用中返回错误（和 n ==
// 0）。 这种一般情况的一个例子就是 Reader
// 在输入流结束时会返回一个非零的字节数， 可能的返回不是 err == EOF
// 就是 err == nil。无论如何，下一个 Read 都应当返回 0,
// EOF。
//
// 调用者应当总在考虑到错误 err 前处理 n > 0
// 的字节。这样做可以在读取一些字节， 以及允许的 EOF
// 行为后正确地处理I/O错误。
//
// Read 的实现在 len(p) == 0 以外的情况下会阻止返回零字节的计数和
// nil 错误， 调用者应将返回 0 和 nil
...
// LimitReader 返回一个 Reader，它从 r 中读取 n 个字节后以
// EOF 停止。 其基本实现为 *LimitedReader。
...
// MultiReader 返回一个 Reader，它是输入 readers
// 提供的的逻辑拼接。 它们按顺序读取。一旦所有的输入返回 EOF，Read
// 就会返回 EOF。 若任何 readers 返回了非 nil 或非 EOF
// 错误，Read 就会返回该错误。
...
// TeeReader 返回一个 Reader，它将从 r 中读到的东西写入 w
...
//
// ReadAt 从基本输入源的偏移量 off 处开始，将 len(p)
// 个字节读取到 p 中。 它返回读取的字节数 n（0 <= n <=
// len(p)）以及任何遇到的错误。
//
...
// 的全部作为暂存空间。 若一些数据可用但不到 len(p) 字节，ReadAt
// 就会阻塞直到所有数据都可用或产生一个错误。 在这一点上 ReadAt 不同于
// Read。
//
// 若 n = len(p) 个字节在输入源的的结尾处由 ReadAt
// 返回，ReadAt 不是返回 err == EOF 就是返回 err ==
// nil。
//
...
// 为读取的字节数。 除 io.EOF 之外，在读取过程中遇到的任何错误也将被返回。
//
...
// RuneScanner 接口将 UnreadRune 方法添加到基本的
// ReadRune 方法。
//
// UnreadRune 使下一次调用 ReadRune 返回的符文与上一次调用
// ReadRune 返回的相同。 调用 UnreadRune 两次而中间没有调用
// ReadRune 的话就会返回错误。
...
// NewSectionReader 返回一个 SectionReader，它从 r
// 中的偏移量 off 处读取 n 个字节后以 EOF 停止。
...
// ，它的解释取决于 whence： 0 表示相对于文件的起始处，1
// 表示相对于当前的偏移，而 2 表示相对于其结尾处。 Seek
// 返回新的偏移量和一个错误，如果有的话。
//
...
// MultiWriter 创建一个 Writer，它将其写入复制到所有提供的
// writers 中，类似于Unix的tee(1)命令。
...
// len(p)）以及任何遇到的引起写入提前停止的错误。 若 WriteAt 返回的
// n < len(p)，它就必须返回一个非nil的错误。
//
// 若 WriteAt 按查找偏移量写入到目标中，WriteAt
// 应当既不影响基本查找偏移量也不被它所影响。
//
// 若区域没有重叠，WriteAt 的客户端可对相同的目标并行执行 WriteAt
// 调用。
//
// 实现必须不保留 p。
...
//
// WriteTo 将数据写入 w 中，直到没有数据可读或发生错误。其返回值 n
