...
// multipart parsing, as defined in RFC
// 2046.
//
// The implementation is sufficient for
// HTTP (RFC 2388) and the multipart bodies
// generated by popular browsers.
package multipart
...
// File is an interface to access the file
// part of a multipart message. Its
// contents may be either stored in memory
// or on disk. If stored on disk, the
// File's underlying concrete type will be
// an *os.File.
type File interface {
	io.Reader
	io.ReaderAt
...
// A FileHeader describes a file part of a
// multipart request.
...
// Open opens and returns the FileHeader's
// associated File.
...
// Form is a parsed multipart form. Its
// File parts are stored either in memory
// or on disk, and are accessible via the
// *FileHeader's Open method. Its Value
// parts are stored as strings. Both are
// keyed by field name.
type Form struct {
...
// RemoveAll removes any temporary files
// associated with a Form.
func (f *Form) RemoveAll() error

// A Part represents a single part in a multipart body.

// A Part represents a single part in a
// multipart body.
type Part struct {
...
// of the Part's Content-Disposition
// header.
...
// FormName returns the name parameter if p
// has a Content-Disposition of type
// "form-data". Otherwise it returns the
// empty string.
...
// Read reads the body of a part, after its
// headers and before the next part (if
// any) begins.
...
// Reader is an iterator over parts in a
// MIME multipart body. Reader's underlying
// parser consumes its input as needed.
...
// NewReader creates a new multipart Reader
// reading from r using the given MIME
// boundary.
//
// The boundary is usually obtained from
// the "boundary" parameter of the
// message's "Content-Type" header. Use
// mime.ParseMediaType to parse such
// headers.
...
// NextPart returns the next part in the
// multipart or an error. When there are no
// more parts, the error io.EOF is
// returned.
...
// ReadForm parses an entire multipart
// message whose parts have a
// Content-Disposition of "form-data". It
// stores up to maxMemory bytes of the file
// parts in memory and the remainder on
...
// NewWriter returns a new multipart Writer
// with a random boundary, writing to w.
...
// Close finishes the multipart message and
// writes the trailing boundary end line to
// the output.
func (w *Writer) Close() error

// CreateFormField calls CreatePart with a header using the given field name.

// CreateFormField calls CreatePart with a
// header using the given field name.
...
// CreateFormFile is a convenience wrapper
// around CreatePart. It creates a new
// form-data header with the provided field
// name and file name.
...
// CreatePart creates a new multipart
// section with the provided header. The
// body of the part should be written to
// the returned Writer. After calling
// CreatePart, any previous part may no
...
// FormDataContentType returns the
// Content-Type for an HTTP
// multipart/form-data with this Writer's
// Boundary.
...
// SetBoundary overrides the Writer's
// default randomly-generated boundary
// separator with an explicit value.
//
// SetBoundary must be called before any
// parts are created, may only contain
// certain ASCII characters, and must be
// 1-69 bytes long.
func (w *Writer) SetBoundary(boundary string) error

// WriteField calls CreateFormField and then writes the given value.

// WriteField calls CreateFormField and
// then writes the given value.
