...
// AddExtensionType sets the MIME type
// associated with the extension ext to
// typ. The extension should begin with a
// leading dot, as in ".html".
...
// FormatMediaType serializes mediatype t
// and the parameters param as a media type
// conforming to RFC 2045 and RFC 2616. The
// type and parameter names are written in
// lower-case. When any of the arguments
// result in a standard violation then
// FormatMediaType returns the empty
// string.
...
// ParseMediaType parses a media type value
// and any optional parameters, per RFC
// 1521. Media types are the values in
// Content-Type and Content-Disposition
// headers (RFC 2183). On success,
// ParseMediaType returns the media type
// converted to lowercase and trimmed of
// white space and a non-nil map. The
// returned map, params, maps from the
// lowercase attribute to the attribute
// value with its case preserved.
...
// TypeByExtension returns the MIME type
// associated with the file extension ext.
// The extension ext should begin with a
// leading dot, as in ".html". When ext has
// no associated type, TypeByExtension
// returns "".
//
// Extensions are looked up first
// case-sensitively, then
// case-insensitively.
//
// The built-in table is small but on unix
// it is augmented by the local system's
// mime.types file(s) if available under
...
//	/etc/apache/mime.types
//
// On Windows, MIME types are extracted
// from the registry.
//
// Text types have the charset parameter
