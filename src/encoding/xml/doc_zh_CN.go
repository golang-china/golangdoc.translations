...
// Package xml implements a simple XML 1.0
// parser that understands XML name spaces.
package xml
...
// HTMLAutoClose is the set of HTML
// elements that should be considered to
// close automatically.
...
// HTMLEntity is an entity map containing
// translations for the standard HTML
// entity characters.
...
// Escape is like EscapeText but omits the
// error return value. It is provided for
// backwards compatibility with Go 1.0.
// Code targeting Go 1.1 or later should
// use EscapeText.
...
// escaped XML equivalent of the plain text
// data s.
...
// Marshal returns the XML encoding of v.
//
// Marshal handles an array or slice by
// marshalling each of the elements.
// Marshal handles a pointer by marshalling
// the value it points at or, if the
// pointer is nil, by writing nothing.
// Marshal handles an interface value by
// marshalling the value it contains or, if
// the interface value is nil, by writing
// nothing. Marshal handles all other data
// by writing one or more XML elements
// containing the data.
//
// The name for the XML elements is taken
...
//	- the name of the marshalled type
//
// The XML element for a struct contains
// marshalled elements for each of the
// exported fields of the struct, with
// these exceptions:
//
//	- the XMLName field, described above, is omitted.
//	- a field with tag "-" is omitted.
...
//	  value were part of the outer struct.
//
// If a field uses a tag "a>b>c", then the
// element c will be nested inside parent
// elements a and b. Fields that appear
// next to each other that name the same
// parent will be enclosed in one XML
// element.
//
// See MarshalIndent for an example.
//
// Marshal will return an error if asked to
// marshal a channel, function, or map.
...
// MarshalIndent works like Marshal, but
// each XML element begins on a new
// indented line that starts with prefix
// and is followed by one or more copies of
...
// Unmarshal parses the XML-encoded data
// and stores the result in the value
// pointed to by v, which must be an
// arbitrary struct, slice, or string.
// Well-formed data that does not fit into
// v is discarded.
//
// Because Unmarshal uses the reflect
// package, it can only assign to exported
// (upper case) fields. Unmarshal uses a
// case-sensitive comparison to match XML
// element names to tag values and struct
// field names.
//
// Unmarshal maps an XML element to a
// struct using the following rules. In the
// rules, the tag of a field refers to the
// value associated with the key 'xml' in
// the struct field's tag (see the example
// above).
//
//	* If the struct has a field of type []byte or string with tag
//	   ",innerxml", Unmarshal accumulates the raw XML nested inside the
...
//
// Unmarshal maps an XML element to a
// string or []byte by saving the
// concatenation of that element's
// character data in the string or []byte.
// The saved []byte is never nil.
//
// Unmarshal maps an attribute value to a
// string or []byte by saving the value in
// the string or slice.
//
// Unmarshal maps an XML element to a slice
// by extending the length of the slice and
// mapping the element to the newly created
// value.
//
// Unmarshal maps an XML element or
// attribute value to a bool by setting it
// to the boolean value represented by the
// string.
//
// Unmarshal maps an XML element or
// attribute value to an integer or
// floating-point field by setting the
// field to the result of interpreting the
// string value in decimal. There is no
// check for overflow.
//
// Unmarshal maps an XML element to an
// xml.Name by recording the element name.
//
// Unmarshal maps an XML element to a
// pointer by setting the pointer to a
// freshly allocated value and then mapping
// the element to that value.
func Unmarshal(data []byte, v interface{}) error

// An Attr represents an attribute in an XML element (Name=Value).

// An Attr represents an attribute in an
...
// A CharData represents XML character data
// (raw text), in which XML escape
// sequences have been replaced by the
...
// A Comment represents an XML comment of
// the form <!--comment-->. The bytes do
// not include the <!-- and --> comment
// markers.
type Comment []byte
...
// A Decoder represents an XML parser
// reading a particular input stream. The
// parser assumes that its input is encoded
// in UTF-8.
type Decoder struct {
...
// reading from r. If r does not implement
// io.ByteReader, NewDecoder will do its
// own buffering.
...
// Decode works like xml.Unmarshal, except
// it reads the decoder stream to find the
// start element.
...
// DecodeElement works like xml.Unmarshal
// except that it takes a pointer to the
// start XML element to decode into v. It
// is useful when a client reads some raw
// XML tokens itself but also wants to
// defer to Unmarshal for some elements.
...
// InputOffset returns the input stream
// byte offset of the current decoder
// position. The offset gives the location
// of the end of the most recently returned
// token and the beginning of the next
// token.
...
// RawToken is like Token but does not
// verify that start and end elements match
// and does not translate name space
...
// Skip reads tokens until it has consumed
// the end element matching the most recent
// start element already consumed. It
// recurs if it encounters a start element,
// so it can be used to skip nested
// structures. It returns nil if it finds
// an end element matching the start
...
// Token returns the next XML token in the
// input stream. At the end of the input
// stream, Token returns nil, io.EOF.
//
// Slices of bytes in the returned token
// data refer to the parser's internal
// buffer and remain valid only until the
// next call to Token. To acquire a copy of
// the bytes, call CopyToken or the token's
// Copy method.
//
// Token expands self-closing elements such
// as <br/> into separate start and end
// elements returned by successive calls.
//
// Token guarantees that the StartElement
// and EndElement tokens it returns are
// properly nested and matched: if Token
// encounters an unexpected end element, it
// will return an error.
//
// Token implements XML name spaces as
// described by
// http://www.w3.org/TR/REC-xml-names/.
// Each of the Name structures contained in
// the Token has the Space set to the URL
// identifying its name space when known.
// If Token encounters an unrecognized name
// space prefix, it uses the prefix as the
// Space rather than report an error.
...
// of the form <!text>. The bytes do not
// include the <! and > markers.
type Directive []byte
...
// An Encoder writes XML data to an output
// stream.
type Encoder struct {
	// contains filtered or unexported fields
}
...
// the stream.
//
// See the documentation for Marshal for
// details about the conversion of Go
// values to XML.
//
// Encode calls Flush before returning.
...
// EncodeElement writes the XML encoding of
// v to the stream, using start as the
// outermost tag in the encoding.
//
// See the documentation for Marshal for
// details about the conversion of Go
// values to XML.
//
// EncodeElement calls Flush before
// returning.
...
// EncodeToken writes the given XML token
// to the stream. It returns an error if
// StartElement and EndElement tokens are
// not properly matched.
//
// EncodeToken does not call Flush, because
// usually it is part of a larger operation
// such as Encode or EncodeElement (or a
// custom Marshaler's MarshalXML invoked
// during those), and those will call Flush
// when finished. Callers that create an
// Encoder and then invoke EncodeToken
// directly, without using Encode or
// EncodeElement, need to call Flush when
// finished to ensure that the XML is
// written to the underlying writer.
//
// EncodeToken allows writing a ProcInst
// with Target set to "xml" only as the
...
// Flush flushes any buffered XML to the
// underlying writer. See the EncodeToken
// documentation for details about when it
// is necessary.
...
// Indent sets the encoder to generate XML
// in which each element begins on a new
// indented line that starts with prefix
// and is followed by one or more copies of
// indent according to the nesting depth.
...
// An EndElement represents an XML end
// element.
...
// by objects that can marshal themselves
// into valid XML elements.
//
// MarshalXML encodes the receiver as zero
// or more XML elements. By convention,
// arrays or slices are typically encoded
// as a sequence of elements, one per
// entry. Using start as the element tag is
// not required, but doing so will enable
// Unmarshal to match the XML elements to
// the correct struct field. One common
// implementation strategy is to construct
// a separate value with a layout
// corresponding to the desired XML and
// then to encode it using e.EncodeElement.
// Another common strategy is to use
// repeated calls to e.EncodeToken to
// generate the XML output one token at a
// time. The sequence of encoded tokens
// must make up zero or more valid XML
// elements.
...
// MarshalerAttr is the interface
// implemented by objects that can marshal
// themselves into valid XML attributes.
//
// MarshalXMLAttr returns an XML attribute
// with the encoded value of the receiver.
// Using name as the attribute name is not
// required, but doing so will enable
// Unmarshal to match the attribute to the
// correct struct field. If MarshalXMLAttr
// returns the zero attribute Attr{}, no
// attribute will be generated in the
// output. MarshalXMLAttr is used only for
// struct fields with the "attr" option in
// the field tag.
...
// A Name represents an XML name (Local)
// annotated with a name space identifier
// (Space). In tokens returned by
// Decoder.Token, the Space identifier is
// given as a canonical URL, not the short
// prefix used in the document being
// parsed.
type Name struct {
	Space, Local string
}
...
// A ProcInst represents an XML processing
// instruction of the form <?target inst?>
...
// A StartElement represents an XML start
// element.
...
// End returns the corresponding XML end
// element.
func (e StartElement) End() EndElement

// A SyntaxError represents a syntax error in the XML input stream.

// A SyntaxError represents a syntax error
...
// A TagPathError represents an error in
// the unmarshalling process caused by the
// use of field tags with conflicting
// paths.
...
// A Token is an interface holding one of
// the token types: StartElement,
// EndElement, CharData, Comment, ProcInst,
// or Directive.
...
// An UnmarshalError represents an error in
...
// by objects that can unmarshal an XML
// element description of themselves.
//
// UnmarshalXML decodes a single XML
// element beginning with the given start
// element. If it returns an error, the
// outer call to Unmarshal stops and
// returns that error. UnmarshalXML must
// consume exactly one XML element. One
// common implementation strategy is to
// unmarshal into a separate value with a
// layout matching the expected XML using
// d.DecodeElement, and then to copy the
// data from that value into the receiver.
// Another common strategy is to use
// d.Token to process the XML object one
// token at a time. UnmarshalXML may not
// use d.RawToken.
...
// UnmarshalerAttr is the interface
// implemented by objects that can
// unmarshal an XML attribute description
// of themselves.
//
// UnmarshalXMLAttr decodes a single XML
// attribute. If it returns an error, the
// outer call to Unmarshal stops and
// returns that error. UnmarshalXMLAttr is
// used only for struct fields with the
...
// A MarshalXMLError is returned when
// Marshal encounters a type that cannot be
// converted into XML.
