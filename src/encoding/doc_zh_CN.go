...
// encoding/gob、encoding/json、encoding/xml三个包都会检查使用这些接口。因此，只
// 要实现了这些接口一次，就可以在多个包里使用。标准包内建类型time.Time和net.IP都
// 实现了这些接口。接口是成对的，分别产生和还原编码后的数据。
package encoding
...
//
// UnmarshalBinary必须可以解码MarshalBinary生成的binary格式数据。本函数可能会对
// data内容作出修改，所以如果要保持data的数据请事先进行拷贝。
...
//
// UnmarshalText必须可以解码MarshalText生成的textual格式数据。本函数可能会对data
// 内容作出修改，所以如果要保持data的数据请事先进行拷贝。
