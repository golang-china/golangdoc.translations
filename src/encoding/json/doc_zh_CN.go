...
// HTMLEscape 函数将json编码的src中的<、>、&、U+2028
// 和U+2029字符替换为\u003c、\u003e、\u0026、\u2028、\u2029
// 转义字符串，以便json编码可以安全的嵌入HTML的<script>标签里。因为历史原因，网络浏览器不支持在<script>标签中使用标准HTML转义，
// 因此必须使用另一种json编码方案。
...
// Go
// 1.2之前版本，当试图编码一个包含非法utf-8序列的字符串时会返回本错误。Go
// 1.2及之后版本，编码器会强行将非法字节替换为unicode字符U+FFFD来使字符串合法。本错误已不会再出现，但出于向后兼容考虑而保留。
