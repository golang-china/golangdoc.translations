...
// Base函数返回路径的最后一个元素。在提取元素前会求掉末尾的斜杠。如果路径是""，
// 会返回"."；如果路径是只有一个斜杆构成，会返回"/"。
...
// 回"."；如果路径由1到多个斜杠后跟0到多个非斜杠字符组成，会返回"/"；其他任何情
// 况下都不会返回以斜杠结尾的路径。
...
// Ext函数返回path文件扩展名。返回值是路径最后一个斜杠分隔出的路径元素的最后一个
// '.'起始的后缀（包括'.'）。如果该元素没有'.'会返回空字符串。
...
// Join函数可以将任意数量的路径元素放入一个单一路径里，会根据需要添加斜杠。结果
// 是经过简化的，所有的空字符串元素会被忽略。
...
// Split函数将路径从最后一个斜杠后面位置分隔为两个部分（dir和file）并返回。如果
// 路径中没有斜杠，函数返回值dir会设为空字符串，file会设为path。两个返回值满足
// path == dir+file。
