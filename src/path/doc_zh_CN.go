...
// Package path implements utility routines
// for manipulating slash-separated paths.
package path
...
// Trailing slashes are removed before
// extracting the last element. If the path
// is empty, Base returns ".". If the path
// consists entirely of slashes, Base
// returns "/".
...
// After dropping the final element using
// Split, the path is Cleaned and trailing
// slashes are removed. If the path is
// empty, Dir returns ".". If the path
// consists entirely of slashes followed by
// non-slash bytes, Dir returns a single
// slash. In any other case, the returned
...
// slash-separated element of path; it is
// empty if there is no dot.
...
// into a single path, adding a separating
// slash if necessary. The result is
// Cleaned; in particular, all empty
// strings are ignored.
...
// the final slash. separating it into a
// directory and file name component. If
// there is no slash path, Split returns an
// empty dir and file set to path. The
// returned values have the property that
// path = dir+file.
