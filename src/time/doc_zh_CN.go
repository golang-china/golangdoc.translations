...
// Package time provides functionality for
// measuring and displaying time.
//
// The calendrical calculations always
...
// Time.Format and Time.Parse. The
// reference time used in the layouts is
// the specific time:
//
//	Mon Jan 2 15:04:05 MST 2006
//
// which is Unix time 1136239445. Since MST
// is GMT-0700, the reference time can be
// thought of as
//
//	01/02 03:04:05PM '06 -0700
//
// To define your own format, write down
// what the reference time would look like
// formatted your way; see the values of
// constants like ANSIC, StampMicro or
// Kitchen for examples. The model is to
// demonstrate what the reference time
// looks like so that the Format and Parse
// methods can apply the same
// transformation to a general time value.
//
// Within the format string, an underscore
// _ represents a space that may be
// replaced by a digit if the following
// number (a day) has two digits; for
// compatibility with fixed-width Unix time
// formats.
//
// A decimal point followed by one or more
// zeros represents a fractional second,
// printed to the given number of decimal
// places. A decimal point followed by one
// or more nines represents a fractional
// second, printed to the given number of
// decimal places, with trailing zeros
// removed. When parsing (only), the input
// may contain a fractional second field
// immediately after the seconds field,
// even if the layout does not signify its
// presence. In that case a decimal point
// followed by a maximal series of digits
// is parsed as a fractional second.
//
// Numeric time zone offsets format as
// follows:
//
//	-0700  ±hhmm
//	-07:00 ±hh:mm
//
// Replacing the sign in the format with a
// Z triggers the ISO 8601 behavior of
// printing Z instead of an offset for the
// UTC zone. Thus:
//
//	Z0700  Z or ±hhmm
//	Z07:00 Z or ±hh:mm
...
// and then sends the current time on the
// returned channel. It is equivalent to
// NewTimer(d).C.
...
// Sleep pauses the current goroutine for
// at least the duration d. A negative or
// zero duration causes Sleep to return
// immediately.
...
// NewTicker providing access to the
// ticking channel only. Useful for clients
// that have no need to shut down the
// ticker.
...
// A Duration represents the elapsed time
// between two instants as an int64
// nanosecond count. The representation
// limits the largest representable
...
// Common durations. There is no definition
// for units of Day or larger to avoid
// confusion across daylight savings time
// zone transitions.
//
...
//	fmt.Print(int64(second/time.Millisecond)) // prints 1000
//
// To convert an integer number of units to
// a Duration, multiply:
//
//	seconds := 10
//	fmt.Print(time.Duration(seconds)*time.Second) // prints 10s
...
// Hours returns the duration as a floating
// point number of hours.
func (d Duration) Hours() float64

// Minutes returns the duration as a floating point number of minutes.

// Minutes returns the duration as a
// floating point number of minutes.
func (d Duration) Minutes() float64

// Nanoseconds returns the duration as an integer nanosecond count.

// Nanoseconds returns the duration as an
// integer nanosecond count.
func (d Duration) Nanoseconds() int64

// Seconds returns the duration as a floating point number of seconds.

// Seconds returns the duration as a
...
// String returns a string representing the
// duration in the form "72h3m0.5s".
// Leading zero units are omitted. As a
// special case, durations less than one
// second format use a smaller unit
// (milli-, micro-, or nanoseconds) to
// ensure that the leading digit is
// non-zero. The zero duration formats as
// 0, with no unit.
...
// A Location maps time instants to the
// zone in use at that time. Typically, the
// Location represents the collection of
// time offsets in use in a geographical
// area, such as CEST and CET for central
// Europe.
...
// Local represents the system's local time
// zone.
var Local *Location = &localLoc

// UTC represents Universal Coordinated Time (UTC).

// UTC represents Universal Coordinated
// Time (UTC).
...
// FixedZone returns a Location that always
// uses the given zone name and offset
// (seconds east of UTC).
...
// LoadLocation returns the Location with
// the given name.
//
// If the name is "" or "UTC", LoadLocation
// returns UTC. If the name is "Local",
// LoadLocation returns Local.
//
// Otherwise, the name is taken to be a
// location name corresponding to a file in
// the IANA Time Zone database, such as
// "America/New_York".
//
// The time zone database needed by
// LoadLocation may not be present on all
// systems, especially non-Unix systems.
// LoadLocation looks in the directory or
// uncompressed zip file named by the
// ZONEINFO environment variable, if any,
// then looks in known installation
// locations on Unix systems, and finally
...
// String returns a descriptive name for
// the time zone information, corresponding
// to the argument to LoadLocation.
func (l *Location) String() string

// A Month specifies a month of the year (January = 1, ...).

// A Month specifies a month of the year
// (January = 1, ...).
type Month int
...
// String returns the English name of the
// month ("January", "February", ...).
func (m Month) String() string

// ParseError describes a problem parsing a time string.

// ParseError describes a problem parsing a
// time string.
...
// A Ticker holds a channel that delivers
// `ticks' of a clock at intervals.
type Ticker struct {
...
// containing a channel that will send the
// time with a period specified by the
// duration argument. It adjusts the
// intervals or drops ticks to make up for
// slow receivers. The duration d must be
// greater than zero; if not, NewTicker
// will panic. Stop the ticker to release
...
// Stop turns off a ticker. After Stop, no
// more ticks will be sent. Stop does not
// close the channel, to prevent a read
...
//
// Programs using times should typically
// store and pass them as values, not
// pointers. That is, time variables and
// struct fields should be of type
// time.Time, not *time.Time. A Time value
// can be used by multiple goroutines
// simultaneously.
//
// Time instants can be compared using the
// Before, After, and Equal methods. The
// Sub method subtracts two instants,
// producing a Duration. The Add method
// adds a Time and a Duration, producing a
// Time.
//
// The zero value of type Time is January
// 1, year 1, 00:00:00.000000000 UTC. As
// this time is unlikely to come up in
// practice, the IsZero method gives a
// simple way of detecting a time that has
// not been initialized explicitly.
//
// Each Time has associated with it a
// Location, consulted when computing the
// presentation form of the time, such as
// in the Format, Hour, and Year methods.
// The methods Local, UTC, and In return a
// Time with a specific location. Changing
// the location in this way changes only
// the presentation; it does not change the
// instant in time being denoted and
// therefore does not affect the
// computations described in earlier
// paragraphs.
//
// Note that the Go == operator compares
// not just the time instant but also the
// Location. Therefore, Time values should
// not be used as map or database keys
// without first guaranteeing that the
// identical Location has been set for all
// values, which can be achieved through
// use of the UTC or Local method.
type Time struct {
	// contains filtered or unexported fields
}
...
//	yyyy-mm-dd hh:mm:ss + nsec nanoseconds
//
// in the appropriate zone for that time in
// the given location.
//
// The month, day, hour, min, sec, and nsec
// values may be outside their usual ranges
// and will be normalized during the
// conversion. For example, October 32
// converts to November 1.
//
// A daylight savings time transition skips
// or repeats times. For example, in the
// United States, March 13, 2011 2:15am
// never occurred, while November 6, 2011
// 1:15am occurred twice. In such cases,
// the choice of time zone, and therefore
// the time, is not well-defined. Date
// returns a time that is correct in one of
// the two zones involved in the
// transition, but it does not guarantee
// which.
//
// Date panics if loc is nil.
...
// Parse parses a formatted string and
// returns the time value it represents.
// The layout defines the format by showing
// how the reference time, defined to be
//
//	Mon Jan 2 15:04:05 -0700 MST 2006
//
// would be interpreted if it were the
// value; it serves as an example of the
// input format. The same interpretation
// will then be made to the input string.
// Predefined layouts ANSIC, UnixDate,
// RFC3339 and others describe standard and
// convenient representations of the
// reference time. For more information
// about the formats and the definition of
// the reference time, see the
// documentation for ANSIC and the other
// constants defined by this package.
//
// Elements omitted from the value are
// assumed to be zero or, when zero is
// impossible, one, so parsing "3:04pm"
// returns the time corresponding to Jan 1,
// year 0, 15:04:00 UTC (note that because
// the year is 0, this time is before the
// zero Time). Years must be in the range
// 0000..9999. The day of the week is
// checked for syntax but it is otherwise
// ignored.
//
// In the absence of a time zone indicator,
// Parse returns a time in UTC.
//
// When parsing a time with a zone offset
// like -0700, if the offset corresponds to
// a time zone used by the current location
// (Local), then Parse uses that location
// and zone in the returned time. Otherwise
// it records the time as being in a
// fabricated location with time fixed at
// the given zone offset.
//
// When parsing a time with a zone
// abbreviation like MST, if the zone
// abbreviation has a defined offset in the
// current location, then that offset is
// used. The zone abbreviation "UTC" is
// recognized as UTC regardless of
// location. If the zone abbreviation is
// unknown, Parse records the time as being
// in a fabricated location with the given
// zone abbreviation and a zero offset.
// This choice means that such a time can
// be parsed and reformatted with the same
// layout losslessly, but the exact instant
// used in the representation will differ
// by the actual zone offset. To avoid such
// problems, prefer time layouts that use a
// numeric zone offset, or use
// ParseInLocation.
...
// ParseInLocation is like Parse but
// differs in two important ways. First, in
// the absence of time zone information,
// Parse interprets a time as UTC;
// ParseInLocation interprets the time as
// in the given location. Second, when
// given a zone offset or abbreviation,
// Parse tries to match it against the
// Local location; ParseInLocation uses the
// given location.
...
// Unix returns the local Time
// corresponding to the given Unix time,
// sec seconds and nsec nanoseconds since
// January 1, 1970 UTC. It is valid to pass
// nsec outside the range [0, 999999999].
...
// AddDate returns the time corresponding
// to adding the given number of years,
// months, and days to t. For example,
// AddDate(-1, 2, 3) applied to January 1,
// 2011 returns March 4, 2010.
//
// AddDate normalizes its result in the
// same way that Date does, so, for
// example, adding one month to October 31
// yields December 1, the normalized form
// for November 31.
func (t Time) AddDate(years int, months int, days int) Time

// After reports whether the time instant t is after u.

// After reports whether the time instant t
// is after u.
func (t Time) After(u Time) bool

// Before reports whether the time instant t is before u.

// Before reports whether the time instant
// t is before u.
func (t Time) Before(u Time) bool

// Clock returns the hour, minute, and second within the day specified by t.

// Clock returns the hour, minute, and
// second within the day specified by t.
func (t Time) Clock() (hour, min, sec int)

// Date returns the year, month, and day in which t occurs.

// Date returns the year, month, and day in
// which t occurs.
...
// Equal reports whether t and u represent
// the same time instant. Two times can be
// equal even if they are in different
// locations. For example, 6:00 +0200 CEST
// and 4:00 UTC are Equal. This comparison
// is different from using t == u, which
// also compares the locations.
...
// of the time value formatted according to
// layout, which defines the format by
// showing how the reference time, defined
// to be
//
//	Mon Jan 2 15:04:05 -0700 MST 2006
//
// would be displayed if it were the value;
// it serves as an example of the desired
// output. The same display rules will then
// be applied to the time value. Predefined
// layouts ANSIC, UnixDate, RFC3339 and
// others describe standard and convenient
// representations of the reference time.
// For more information about the formats
// and the definition of the reference
// time, see the documentation for ANSIC
// and the other constants defined by this
// package.
...
// specified by t, in the range [0, 23].
...
// ISOWeek returns the ISO 8601 year and
// week number in which t occurs. Week
// ranges from 1 to 53. Jan 01 to Jan 03 of
// year n might belong to week 52 or 53 of
// year n-1, and Dec 29 to Dec 31 might
// belong to week 1 of year n+1.
...
// In returns t with the location
// information set to loc.
//
// In panics if loc is nil.
...
// IsZero reports whether t represents the
// zero time instant, January 1, year 1,
// 00:00:00 UTC.
func (t Time) IsZero() bool

// Local returns t with the location set to local time.

// Local returns t with the location set to
// local time.
func (t Time) Local() Time

// Location returns the time zone information associated with t.

// Location returns the time zone
// information associated with t.
...
// json.Marshaler interface. The time is a
// quoted string in RFC 3339 format, with
...
// time is formatted in RFC 3339 format,
// with sub-second precision added if
// present.
...
// Minute returns the minute offset within
// the hour specified by t, in the range
// [0, 59].
...
// within the second specified by t, in the
// range [0, 999999999].
...
// Round returns the result of rounding t
// to the nearest multiple of d (since the
// zero time). The rounding behavior for
// halfway values is to round up. If d <=
...
// Second returns the second offset within
// the minute specified by t, in the range
// [0, 59].
...
// String returns the time formatted using
// the format string
//
//	"2006-01-02 15:04:05.999999999 -0700 MST"
...
// Sub returns the duration t-u. If the
// result exceeds the maximum (or minimum)
// value that can be stored in a Duration,
// the maximum (or minimum) duration will
// be returned. To compute t-d for a
...
// Truncate returns the result of rounding
// t down to a multiple of d (since the
// zero time). If d <= 0, Truncate returns
// t unchanged.
func (t Time) Truncate(d Duration) Time

// UTC returns t with the location set to UTC.

// UTC returns t with the location set to
// UTC.
...
// Unix returns t as a Unix time, the
...
// UnixNano returns t as a Unix time, the
// number of nanoseconds elapsed since
// January 1, 1970 UTC. The result is
// undefined if the Unix time in
// nanoseconds cannot be represented by an
// int64. Note that this means the result
// of calling UnixNano on the zero Time is
// undefined.
...
// json.Unmarshaler interface. The time is
// expected to be a quoted string in RFC
// 3339 format.
...
// time is expected to be in RFC 3339
// format.
...
// specified by t, in the range [1,365] for
// non-leap years, and [1,366] in leap
// years.
...
// Zone computes the time zone in effect at
// time t, returning the abbreviated name
// of the zone (such as "CET") and its
...
// The Timer type represents a single
// event. When the Timer expires, the
// current time will be sent on C, unless
// the Timer was created by AfterFunc. A
// Timer must be created with NewTimer or
// AfterFunc.
type Timer struct {
	C <-chan Time
	// contains filtered or unexported fields
...
// AfterFunc waits for the duration to
// elapse and then calls f in its own
// goroutine. It returns a Timer that can
// be used to cancel the call using its
// Stop method.
...
// NewTimer creates a new Timer that will
// send the current time on its channel
// after at least duration d.
...
// Reset changes the timer to expire after
// duration d. It returns true if the timer
// had been active, false if the timer had
...
// Stop prevents the Timer from firing. It
// returns true if the call stops the
// timer, false if the timer has already
// expired or been stopped. Stop does not
// close the channel, to prevent a read
// from the channel succeeding incorrectly.
