...
// HTML输出。本包提供了和text/template包相同的接口，无论何时当输出是HTML的时候都
// 应使用本包。
//
...
// HTML用于封装一个已知安全的HTML文档片段。它不应被第三方使用，也不能用于含有未
// 闭合的标签或注释的HTML文本。该类型适用于封装一个效果良好的HTML生成器生成的
// HTML文本或者本包模板的输出的文本。
type HTML string
...
// JS用于封装一个已知安全的EcmaScript5表达式，如`(x + y * z())`。模板作者有责任
// 确保封装的字符串不会破坏原有的语义，也不能包含有歧义的声明或表达式，如"{ foo:
// bar() }\n['foo']()"，这一句既是合法的表达式也是语义完全不同的合法程序。
type JS string

// JSStr用于封装一个打算嵌入JavaScript表达式中的字符序列，该字符串必须匹配一系列
// StringCharacters：
//
...
//
// 形如`javascript:checkThatFormNotEditedBeforeLeavingPage()`的来源可信的URL应写
// 进页面里，但一般动态的`javascript:` URL排除在外（不写进页面），因为它们是频繁
// 使用的注入向量。
type URL string
