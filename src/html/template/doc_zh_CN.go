...
// JS用于封装一个已知安全的EcmaScript5表达式，如`(x + y *
// z())`。模板作者有责任确保封装的字符串不会破坏原有的语义，也不能包含有歧义的声明或表达式，如"{ foo: bar()
...
// New方法创建一个和t关联的名字为name的模板并返回它。这种可以传递的关联允许一个模板使用template
// action调用另一个模板。
...
// ParseGlob方法解析filenames指定的文件里的模板定义并将解析结果与t关联。如果发生错误，会停止解析并返回nil，否则返回(t,
// nil)。至少要提供一个文件。
...
// ParseFiles方法解析匹配pattern的文件里的模板定义并将解析结果与t关联。如果发生错误，会停止解析并返回nil，否则返回(t,
// nil)。至少要存在一个匹配的文件。
...
//
// 形如`javascript:checkThatFormNotEditedBeforeLeavingPage()`的来源可信的URL应写进页面里，但一般动态的`javascript:`
// URL排除在外（不写进页面），因为它们是频繁使用的注入向量。
type URL string
