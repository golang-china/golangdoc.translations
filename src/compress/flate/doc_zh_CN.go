...
//
// 和 zlib 包一样, level 的范围是 1 (BestSpeed) 到9 (BestCompression); 值越大,
// 压缩效果越好, 但也越慢; level 为 0 表示不尝试做任何压缩, 只添加必需的 deflate
// 框架; level 为 -1 时会使用默认的压缩水平; 如果 level 在 [-1, 9] 范围内, error
// 返回值将是 nil, 否则将返回非 nil 的错误值.
