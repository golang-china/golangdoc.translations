...
// Package flate implements the DEFLATE
// compressed data format, described in RFC
// 1951. The gzip and zlib packages
...
// NewReader returns a new ReadCloser that
// can be used to read the uncompressed
// version of r. If r does not also
...
//
// The ReadCloser returned by NewReader
...
// NewReaderDict is like NewReader but
// initializes the reader with a preset
// dictionary. The returned Reader behaves
// as if the uncompressed data stream
// started with the given dictionary, which
// has already been read. NewReaderDict is
// typically used to read data compressed
// by NewWriterDict.
//
// The ReadCloser returned by NewReader
// also implements Resetter.
...
// A CorruptInputError reports the presence
// of corrupt input at a given offset.
...
// An InternalError reports an error in the
// flate code itself.
...
// A ReadError reports an error encountered
// while reading input.
...
// The actual read interface needed by
// NewReader. If the passed in io.Reader
// does not also have ReadByte, the
// NewReader will introduce its own
// buffering.
...
// Resetter resets a ReadCloser returned by
// NewReader or NewReaderDict to to switch
// to a new underlying Reader. This permits
// reusing a ReadCloser instead of
// allocating a new one.
...
// A WriteError reports an error
// encountered while writing output.
...
// A Writer takes data written to it and
// writes the compressed form of that data
...
// compressing data at the given level.
// Following zlib, levels range from 1
// (BestSpeed) to 9 (BestCompression);
// higher levels typically run slower but
// compress more. Level 0 (NoCompression)
// does not attempt any compression; it
// only adds the necessary DEFLATE framing.
// Level -1 (DefaultCompression) uses the
// default compression level.
//
// If level is in the range [-1, 9] then
// the error returned will be nil.
// Otherwise the error returned will be
// non-nil.
...
// NewWriterDict is like NewWriter but
// initializes the new Writer with a preset
// dictionary. The returned Writer behaves
// as if the dictionary had been written to
// it without producing any compressed
// output. The compressed data written to w
// can only be decompressed by a Reader
...
// data to the underlying writer. It is
// useful mainly in compressed network
// protocols, to ensure that a remote
// reader has enough data to reconstruct a
// packet. Flush does not return until the
// data has been written. If the underlying
// writer returns an error, Flush returns
// that error.
//
...
// Reset discards the writer's state and
// makes it equivalent to the result of
// NewWriter or NewWriterDict called with
// dst and w's level and dictionary.
func (w *Writer) Reset(dst io.Writer)

// Write writes data to w, which will eventually write the compressed form of data
// to its underlying writer.

// Write writes data to w, which will
// eventually write the compressed form of
