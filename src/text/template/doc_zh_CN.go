...
// pipeline characters '|'. In a chained pipeline, the result of the each
// command is passed as the last argument of the following command. The output
// of the final command in the pipeline is the value of the pipeline.
//
...
//
// 如果要生成HTML格式的输出, 参见html/template包, 该包提供了和本包相同的接口, 但
// 会自动将输出转化为安全的HTML格式输出, 可以抵抗一些网络攻击.
//
// 通过将模板应用于一个数据结构(即该数据结构作为模板的参数)来执行, 来获得输出.
// 模板中的注释引用数据接口的元素(一般如结构体的字段或者字典的键)来控制执行过程
// 和获取需要呈现的值. 模板执行时会遍历结构并将指针表示为 '.'(称之为"dot")指向运
// 行过程中数据结构的当前位置的值.
//
// 用作模板的输入文本必须是 utf-8 编码的文本. "Action"—数据运算和控制单位—由
// "{{" 和 "}}" 界定. 在Action之外的所有文本都不做修改的拷贝到输出中. Action内部
// 不能有换行, 但注释可以有换行.
//
