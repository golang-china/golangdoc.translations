...
// tabwriter包实现了写入过滤器（tabwriter.Writer），可以将输入的缩进修正为正确的
// 对齐文本。
//
...
// the tab in this string "Ignore this tab: \xff\t\xff" does not terminate a
...
//
// 它会将输入的序列视为utf-8编码的文本，包含一系列被垂直制表符、水平制表符、换行
// 符、回车符分割的单元。临近的单元组成一列，根据需要填充空格使所有的单元有相同
// 的宽度，高效对齐各列。它假设所有的字符都有相同的宽度，除了tab，tab宽度应该被
// 指定。注意单元以tab截止，而不是被tab分隔，行最后的非tab文本不被视为列的单元。
//
// Writer假设所有的unicode字符有着同样的宽度，这一点其实在很多字体里是错误的。
//
// 如果设置了DiscardEmptyColumns，以垂直制表符结尾的空列会被丢弃，水平制表符截止
// 的空列则不会被影响。
//
// 如果设置了FilterHTML，HTML标签和实体会被放过，标签宽度视为0，实体宽度视为1。
// 文本段可能被转义字符包围，此时tabwriter不会修改该文本段，不会打断段中的任何
// tab或换行。
//
// 如果设置了StripEscape，则不会计算转义字符的宽度（输出中也会去除转义字符）。
//
// 进纸符'\f'被视为换行，但也会截止当前行的所有列（有效的刷新）；除非在HTML标签
// 内或者在转义文本段内，输出中'\f'都被作为换行。
//
