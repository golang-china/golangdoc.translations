...
// that the address of a variable with the type of v will always be zero mod m. If
...
// Sizeof returns the size in bytes occupied by the value v. The size is that of
// the "top level" of the value only. For instance, if v is a slice, it returns the
// size of the slice descriptor, not the size of the memory referenced by the
// slice.
