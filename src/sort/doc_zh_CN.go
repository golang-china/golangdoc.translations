...
// 假定该索引在区间 [0, n) 内，则 f(i) == true 就蕴含了 f(i+1) == true。 也就是
// 说，Search 要求 f 对于输入区间 [0, n) （可能为空）的前一部分为 false， 而对于
// 剩余（可能为空）的部分为 true；Search 返回第一个 f 为 true 时的索引 i。 若该
// 索引不存在，Search 就返回 n。（注意，“未找到”的返回值并不像 strings.Index
// 这类函数一样返回 -1）。Search 仅当 i 在区间 [0, n) 内时才调用 f(i)。
//
// Search 常用于在一个已排序的，可索引的数据结构中寻找索引为 i 的值 x，例如数组
// 或切片。 这种情况下，实参 f，一般是一个闭包，会捕获所要搜索的值，以及索引并排
// 序该数据结构的方式。
//
