...
// 其值可能会被覆盖（而内存则会被重用）。为保留操作，其结果也会被返回。若该方法返回除 *Int 或 *Rat
// 之外的结果，其中一个操作数将被作为接收者。
package big
...
// DivMod 在 y != 0 时，将 z 置为 x 除以 y 的商，将 m 置为 x 取模 y
// 的模数并返回值对 (z, m)。 若 y ==
// 0，就会产生一个除以零的运行时派错。
//
...
// 的欧氏定义》以及《ACM编程语言与系统会议记录》 （TOPLAS），14(2):127-144, New York, NY, USA, 4/1992. ACM
// 出版社。） 截断式除法和取模（与Go相同）见 QuoRem。
...
// Exp 置 z = x**y mod |m|（换言之，m 的符号被忽略），并返回 z。 若 y <=0，则其结果为 1，若 m == nil 或 m ==
// 0，则 z = x**y。 见 Knuth《计算机程序设计艺术》，卷 2，章节 4.6.3。
...
// GCD 将 z 置为 a 和 b 的最大公约数，二者必须均 > 0，并返回 z。 若 x 或 y 非 nil，GCD 会设置 x 与 y
// 的值使得 z = a*x + b*y。 若 a 或 b <= 0，GCD就会置 z = x = y = 0。
...
// ProbablyPrime 通过执行 n 次 Miller-Rabin 测试来检查 x 是否为质数。 若它返回 true，x 有 1 - 1/4^n
...
// QuoRem 在 y != 0 时，将 z 置为 x/y 的商，将 r 置为 x%y 的余数并返回值对 (z, r)。 若 y ==
// 0，就会产生一个除以零的运行时派错。
//
...
//
// （详见 Daan
// Leijen，《计算机科学家的除法和取模》。） 欧氏除法和取模（与Go不同）见 DivMod。
...
// SetBit 将 z 置为 x，将 x 的第 i 位置为 b（0 或 1）。 换言之，若 b 为 1，SetBit 会置 z = x | (1 <<
// i)；若 b 为 0，SetBit 会置 z = x &^ (1 << i)。若 b 非 0 或 1，SetBit
// 就会引发派错。
...
// SetBits 提供了对 z
// 的原始访问（未经检查但很快）。它通过将其值设为 abs，解释为小端序的 Word 切片，并返回 z 来实现。SetBits
// 旨在支持此包外缺失的底层 Int
// 功能的实现，除此之外应尽量避免。
...
// SetString 将 z 置为 s 的值，按给定的进制 base 解释并返回 z
// 和一个指示是否成功的布尔值。 若 SetString 失败，则 z
// 的值是未定义的，其返回值则为 nil。
//
// 进制实参 base 必须为 0 或从 2 到 MaxBase 的值。若 base 为
// 0，则其实际的转换进制由
