...
// 本包提供：
//
// 错误，代表服务端回复的错误码。Pipeline，以管理客户端中的管道化的请求/回复。
// Reader，读取数值回复码行，键值对形式的头域，一个作为后续行先导的空行，以及以
// 只有一个"."的一行为结尾的整个文本块。Writer，写入点编码的文本。Conn，对Reader
// 、Writer和Pipline的易用的包装，用于单个网络连接。
package textproto
...
// Conn代表一个文本网络协议的连接。它包含一个Reader和一个Writer来管理读写，一个
// Pipeline来对连接中并行的请求进行排序。匿名嵌入的类型字段是Conn可以调用它们的
// 方法。
type Conn struct {
	Reader
	Writer
...
// 返回一个MIME头的键的规范格式。该标准会将首字母和所有"-"之后的字符改为大写，其
// 余字母改为小写。举个例子，"accept-encoding"作为键的标准格式是
// "Accept-Encoding"。MIME头的键必须是ASCII码构成。
func CanonicalMIMEHeaderKey(s string) string

// Dial connects to the given address on the given network using net.Dial
// and then returns a new Conn for the connection.

// Dial函数使用net.Dial在给定网络上和给定地址建立网络连接，并返回用于该连接的
// Conn。
...
// Cmd方法用于在管道中等待轮到它执行，并发送命令。命令文本是用给定的format字符串
// 和参数格式化生成的。并会在最后添加上\r\n。Cmd函数返回该命令的Pipeline id，用
// 于StartResponse和EndResponse方法。
//
...
// DotReader方法返回一个io.Reader，该接口自动解码r中读取的点编码块。注意该接口仅
// 在下一次调用r的方法之前才有效。点编码是文本协议如SMTP用于文本块的通用框架。数
// 据包含多个行，每行以"\r\n"结尾。数据本身以一个只含有一个点的一行".\r\n"来结尾
// 。以点起始的行会添加额外的点，来避免看起来像是文本的结尾。
//
// 返回接口的Read方法会将行尾的"\r\n"修改为"\n"，去掉起头的转义点，并在底层读取
// 到（并抛弃掉）表示文本结尾的行时停止解码并返回io.EOF错误。
...
// ReadContinuedLine从r中读取可能有后续的行，会将该行尾段的ASCII空白剔除，并将该
// 行后面所有以空格或者tab起始的行视为其后续，后续部分会剔除行头部的空白，所有这
// 些行包括第一行以单个空格连接起来返回。
//
// 举例如下：
//
...
// ReadDotLines方法读取一个点编码文本块并返回一个包含解码后各行的切片，各行最后
// 的\r\n或\n去掉。
...
// ReadMIMEHeader从r读取MIME风格的头域。该头域包含一系列可能有后续的键值行，以空
// 行结束。返回的map映射CanonicalMIMEHeaderKey(key)到值的序列（顺序与输入相同）
// 。
//
// 举例如下：
//
...
// 	code message line n
//
// 其中code是三位数的状态码。第一行以code和连字符开始，最后以同code后跟空格的行
// 结束。返回值message每行以\n分隔。细节参见RFC
// 959(http://www.ietf.org/rfc/rfc959.txt)第36页。
//
// 如果状态码字符串的前缀不匹配expectCode，方法返回时err设为&Error{code,
// message}。例如expectCode是31，则如果状态码不在区间[310, 319]内就会返回错误。
// 如果回复是多行的则会返回错误。
//
// 如果expectCode <= 0，将不会检查状态码。
...
// DotWriter方法返回一个io.WriteCloser，用于将点编码文本写入w。返回的接口会在必
// 要时添加转义点，将行尾的\n替换为\r\n，并在关闭时添加最后的.\r\n行。调用者必须
// 在下一次调用w的方法前关闭该接口。点编码文本格式参见Reader.DotReader方法。
...
// Get方法返回键对应的值集的第一个值。如果键没有关联值，返回""。如要获得键对应的
// 值集直接用map。
