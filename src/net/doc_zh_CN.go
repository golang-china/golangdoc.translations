...
// interface for network I/O, including
// TCP/IP, UDP, domain name resolution, and
// Unix domain sockets.
//
// Although the package provides access to
// low-level networking primitives, most
// clients will need only the basic
// interface provided by the Dial, Listen,
// and Accept functions and the associated
// Conn and Listener interfaces. The
// crypto/tls package uses the same
// interfaces and similar Dial and Listen
// functions.
//
// The Dial function connects to a server:
//
...
// InterfaceAddrs returns a list of the
// system's network interface addresses.
func InterfaceAddrs() ([]Addr, error)

// Interfaces returns a list of the system's network interfaces.

// Interfaces returns a list of the
...
// JoinHostPort combines host and port into
// a network address of the form
// "host:port" or, if host contains a colon
// or a percent sign, "[host]:port".
func JoinHostPort(host, port string) string

// LookupAddr performs a reverse lookup for the given address, returning a list of
// names mapping to that address.

// LookupAddr performs a reverse lookup for
// the given address, returning a list of
// names mapping to that address.
...
// LookupCNAME returns the canonical DNS
// host for the given name. Callers that do
// not care about the canonical name can
// call LookupHost or LookupIP directly;
// both take care of resolving the
// canonical name as part of the lookup.
func LookupCNAME(name string) (cname string, err error)

// LookupHost looks up the given host using the local resolver. It returns an array
// of that host's addresses.

// LookupHost looks up the given host using
// the local resolver. It returns an array
// of that host's addresses.
...
// LookupIP looks up host using the local
// resolver. It returns an array of that
// host's IPv4 and IPv6 addresses.
...
// LookupMX returns the DNS MX records for
// the given domain name sorted by
// preference.
func LookupMX(name string) (mx []*MX, err error)

// LookupNS returns the DNS NS records for the given domain name.

// LookupNS returns the DNS NS records for
// the given domain name.
func LookupNS(name string) (ns []*NS, err error)

// LookupPort looks up the port for the given network and service.

// LookupPort looks up the port for the
// given network and service.
...
// of the given service, protocol, and
// domain name. The proto is "tcp" or
// "udp". The returned records are sorted
// by priority and randomized by weight
// within a priority.
//
// LookupSRV constructs the DNS name to
// look up following RFC 2782. That is, it
// looks up _service._proto.name. To
// accommodate services publishing SRV
// records under non-standard names, if
// both service and proto are empty
// strings, LookupSRV looks up name
// directly.
func LookupSRV(service, proto, name string) (cname string, addrs []*SRV, err error)

// LookupTXT returns the DNS TXT records for the given domain name.

// LookupTXT returns the DNS TXT records
...
// of the form "host:port", "[host]:port"
// or "[ipv6-host%zone]:port" into host or
// ipv6-host%zone and port. A literal
// address or host name for IPv6 must be
// enclosed in square brackets, as in
// "[::1]:80", "[ipv6-host]:http" or
// "[ipv6-host%zone]:80".
func SplitHostPort(hostport string) (host, port string, err error)

// Addr represents a network end point address.

// Addr represents a network end point
// address.
type Addr interface {
...
// Conn is a generic stream-oriented
// network connection.
//
...
// Dial connects to the address on the
// named network.
//
// Known networks are "tcp", "tcp4"
// (IPv4-only), "tcp6" (IPv6-only), "udp",
// "udp4" (IPv4-only), "udp6" (IPv6-only),
// "ip", "ip4" (IPv4-only), "ip6"
// (IPv6-only), "unix", "unixgram" and
// "unixpacket".
//
// For TCP and UDP networks, addresses have
// the form host:port. If host is a literal
// IPv6 address it must be enclosed in
// square brackets as in "[::1]:80" or
// "[ipv6-host%zone]:80". The functions
// JoinHostPort and SplitHostPort
...
//	Dial("tcp", "[fe80::1%lo0]:80")
//
// For IP networks, the network must be
// "ip", "ip4" or "ip6" followed by a colon
// and a protocol number or name and the
// addr must be a literal IP address.
//
// Examples:
//
//	Dial("ip4:1", "127.0.0.1")
//	Dial("ip6:ospf", "::1")
//
// For Unix networks, the address must be a
// file system path.
...
// DialTimeout acts like Dial but takes a
// timeout. The timeout includes name
// resolution, if required.
...
// connection corresponding to the open
// file f. It is the caller's
// responsibility to close f when finished.
// Closing c does not affect f, and closing
// f does not affect c.
...
// Pipe creates a synchronous, in-memory,
// full duplex network connection; both
// ends implement the Conn interface. Reads
// on one end are matched with writes on
// the other, copying data directly between
// the two; there is no internal buffering.
func Pipe() (Conn, Conn)

// DNSConfigError represents an error reading the machine's DNS configuration.

// DNSConfigError represents an error
// reading the machine's DNS configuration.
...
// A Dialer contains options for connecting
// to an address.
//
// The zero value for each field is
// equivalent to dialing without that
// option. Dialing with the zero value of
// Dialer is therefore equivalent to just
...
// named network.
//
// See func Dial for a description of the
...
// A HardwareAddr represents a physical
// hardware address.
...
// ParseMAC parses s as an IEEE 802 MAC-48,
// EUI-48, or EUI-64 using one of the
// following formats:
//
//	01:23:45:67:89:ab
//	01:23:45:67:89:ab:cd:ef
...
// An IP is a single IP address, a slice of
// bytes. Functions in this package accept
// either 4-byte (IPv4) or 16-byte (IPv6)
// slices as input.
//
// Note that in this documentation,
// referring to an IP address as an IPv4
// address or an IPv6 address is a semantic
// property of the address, not just the
// length of the byte slice: a 16-byte
// slice can still be an IPv4 address.
type IP []byte

// IPv4 returns the IP address (in 16-byte form) of the IPv4 address a.b.c.d.

// IPv4 returns the IP address (in 16-byte
// form) of the IPv4 address a.b.c.d.
...
// ParseCIDR parses s as a CIDR notation IP
// address and mask, like
// "192.168.100.1/24" or "2001:DB8::/48",
// as defined in RFC 4632 and RFC 4291.
//
// It returns the IP address and the
// network implied by the IP and mask. For
// example, ParseCIDR("192.168.100.1/16")
// returns the IP address 192.168.100.1 and
// the network 192.168.0.0/16.
...
// returning the result. The string s can
// be in dotted decimal ("74.125.19.99") or
// IPv6 ("2001:4860:0:2001::68") form. If s
// is not a valid textual representation of
// an IP address, ParseIP returns nil.
...
// DefaultMask returns the default IP mask
// for the IP address ip. Only IPv4
// addresses have default masks;
// DefaultMask returns nil if ip is not a
// valid IPv4 address.
...
// Equal returns true if ip and x are the
// same IP address. An IPv4 address and
// that same address in IPv6 form are
// considered to be equal.
func (ip IP) Equal(x IP) bool

// IsGlobalUnicast returns true if ip is a global unicast address.

// IsGlobalUnicast returns true if ip is a
...
// encoding.TextMarshaler interface. The
// encoding is the same as returned by
// String.
func (ip IP) MarshalText() ([]byte, error)

// Mask returns the result of masking the IP address ip with mask.

// Mask returns the result of masking the
// IP address ip with mask.
...
// String returns the string form of the IP
// address ip. If the address is an IPv4
// address, the string representation is
// dotted decimal ("74.125.19.99").
// Otherwise the representation is IPv6
// ("2001:4860:0:2001::68").
...
// To16 converts the IP address ip to a
// 16-byte representation. If ip is not an
// IP address (it is the wrong length),
// To16 returns nil.
func (ip IP) To16() IP
...
// To4 converts the IPv4 address ip to a
// 4-byte representation. If ip is not an
// IPv4 address, To4 returns nil.
func (ip IP) To4() IP
...
// encoding.TextUnmarshaler interface. The
// IP address is expected in a form
// accepted by ParseIP.
func (ip *IP) UnmarshalText(text []byte) error

// IPAddr represents the address of an IP end point.

// IPAddr represents the address of an IP
// end point.
type IPAddr struct {
	IP   IP
...
// ResolveIPAddr parses addr as an IP
// address of the form "host" or
// "ipv6-host%zone" and resolves the domain
// name on the network net, which must be
// "ip", "ip4" or "ip6".
...
// IPConn is the implementation of the Conn
// and PacketConn interfaces for IP network
// connections.
type IPConn struct {
...
// DialIP connects to the remote address
// raddr on the network protocol netProto,
// which must be "ip", "ip4", or "ip6"
// followed by a colon and a protocol
// number or name.
...
// ListenIP listens for incoming IP packets
// addressed to the local address laddr.
// The returned connection's ReadFrom and
// WriteTo methods can be used to receive
// and send IP packets with per-packet
// addressing.
...
// File sets the underlying os.File to
// blocking mode and returns a copy. It is
// the caller's responsibility to close f
// when finished. Closing c does not affect
// f, and closing f does not affect c.
//
// The returned os.File's file descriptor
// is different from the connection's.
// Attempting to change properties of the
// original using this duplicate may or may
// not have the desired effect.
...
// ReadFrom implements the PacketConn
// ReadFrom method.
...
// ReadFromIP reads an IP packet from c,
// copying the payload into b. It returns
// the number of bytes copied into b and
// the return address that was on the
// packet.
//
// ReadFromIP can be made to time out and
// return an error with Timeout() == true
// after a fixed time limit; see
...
// ReadMsgIP reads a packet from c, copying
// the payload into b and the associated
// out-of-band data into oob. It returns
// the number of bytes copied into b, the
// number of bytes copied into oob, the
// flags that were set on the packet and
...
// SetDeadline implements the Conn
// SetDeadline method.
...
// SetReadBuffer sets the size of the
...
// SetReadDeadline implements the Conn
...
// SetWriteBuffer sets the size of the
// operating system's transmit buffer
// associated with the connection.
func (c *IPConn) SetWriteBuffer(bytes int) error

// SetWriteDeadline implements the Conn SetWriteDeadline method.

// SetWriteDeadline implements the Conn
...
// WriteMsgIP writes a packet to addr via
// c, copying the payload from b and the
// associated out-of-band data from oob. It
// returns the number of payload and
// out-of-band bytes written.
func (c *IPConn) WriteMsgIP(b, oob []byte, addr *IPAddr) (n, oobn int, err error)

// WriteTo implements the PacketConn WriteTo method.

// WriteTo implements the PacketConn
// WriteTo method.
...
// WriteToIP writes an IP packet to addr
// via c, copying the payload from b.
//
// WriteToIP can be made to time out and
// return an error with Timeout() == true
// after a fixed time limit; see
// SetDeadline and SetWriteDeadline. On
// packet-oriented connections, write
// timeouts are rare.
...
// CIDRMask returns an IPMask consisting of
// `ones' 1 bits followed by 0s up to a
// total length of `bits' bits. For a mask
// of this form, CIDRMask is the inverse of
// IPMask.Size.
func CIDRMask(ones, bits int) IPMask

// IPv4Mask returns the IP mask (in 4-byte form) of the IPv4 mask a.b.c.d.

// IPv4Mask returns the IP mask (in 4-byte
// form) of the IPv4 mask a.b.c.d.
...
// Size returns the number of leading ones
// and total bits in the mask. If the mask
// is not in the canonical form--ones
// followed by zeros--then Size returns 0,
// 0.
func (m IPMask) Size() (ones, bits int)

// String returns the hexadecimal form of m, with no punctuation.

// String returns the hexadecimal form of
...
// Contains reports whether the network
// includes ip.
...
// like "192.168.100.1/24" or
// "2001:DB8::/48" as defined in RFC 4632
// and RFC 4291. If the mask is not in the
// canonical form, it returns the string
// which consists of an IP address,
// followed by a slash character and a mask
// expressed as hexadecimal form with no
// punctuation like
// "192.168.100.1/c000ff00".
...
// Interface represents a mapping between
// network interface name and index. It
// also represents network interface
...
// InterfaceByIndex returns the interface
// specified by index.
func InterfaceByIndex(index int) (*Interface, error)

// InterfaceByName returns the interface specified by name.

// InterfaceByName returns the interface
// specified by name.
func InterfaceByName(name string) (*Interface, error)

// Addrs returns interface addresses for a specific interface.

// Addrs returns interface addresses for a
// specific interface.
...
// MulticastAddrs returns multicast, joined
// group addresses for a specific
// interface.
...
// FileListener returns a copy of the
// network listener corresponding to the
// open file f. It is the caller's
// responsibility to close l when finished.
// Closing l does not affect f, and closing
// f does not affect l.
...
// Listen announces on the local network
// address laddr. The network net must be a
// stream-oriented network: "tcp", "tcp4",
// "tcp6", "unix" or "unixpacket". See Dial
// for the syntax of laddr.
...
// OpError is the error type usually
// returned by functions in the net
// package. It describes the operation,
// network type, and address of an error.
type OpError struct {
...
// PacketConn is a generic packet-oriented
// network connection.
//
// Multiple goroutines may invoke methods
// on a PacketConn simultaneously.
...
// FilePacketConn returns a copy of the
// packet network connection corresponding
// to the open file f. It is the caller's
// responsibility to close f when finished.
// Closing c does not affect f, and closing
// f does not affect c.
...
// ListenPacket announces on the local
// network address laddr. The network net
// must be a packet-oriented network:
// "udp", "udp4", "udp6", "ip", "ip4",
// "ip6" or "unixgram". See Dial for the
// syntax of laddr.
...
// A ParseError represents a malformed text
// string and the type of string that was
// expected.
...
// An SRV represents a single DNS SRV
// record.
type SRV struct {
	Target   string
	Port     uint16
...
// TCPAddr represents the address of a TCP
// end point.
type TCPAddr struct {
	IP   IP
	Port int
...
// ResolveTCPAddr parses addr as a TCP
// address of the form "host:port" or
// "[ipv6-host%zone]:port" and resolves a
// pair of domain name and port name on the
// network net, which must be "tcp", "tcp4"
// or "tcp6". A literal address or host
// name for IPv6 must be enclosed in square
// brackets, as in "[::1]:80",
// "[ipv6-host]:http" or
// "[ipv6-host%zone]:80".
...
// TCPConn is an implementation of the Conn
// interface for TCP network connections.
type TCPConn struct {
	// contains filtered or unexported fields
}
...
// DialTCP connects to the remote address
// raddr on the network net, which must be
// "tcp", "tcp4", or "tcp6". If laddr is
// not nil, it is used as the local address
// for the connection.
...
// CloseRead shuts down the reading side of
// the TCP connection. Most callers should
// just use Close.
...
// CloseWrite shuts down the writing side
// of the TCP connection. Most callers
// should just use Close.
...
// SetKeepAlive sets whether the operating
// system should send keepalive messages on
// the connection.
...
// SetLinger sets the behavior of Close on
// a connection which still has data
// waiting to be sent or to be
// acknowledged.
//
// If sec < 0 (the default), the operating
// system finishes sending the data in the
// background.
//
// If sec == 0, the operating system
// discards any unsent or unacknowledged
// data.
//
// If sec > 0, the data is sent in the
// background as with sec < 0. On some
// operating systems after sec seconds have
// elapsed any remaining unsent data may be
// discarded.
...
// SetNoDelay controls whether the
// operating system should delay packet
// transmission in hopes of sending fewer
// packets (Nagle's algorithm). The default
// is true (no delay), meaning that data is
...
// Clients should typically use variables
// of type Listener instead of assuming
// TCP.
...
// ListenTCP announces on the TCP address
// laddr and returns a TCP listener. Net
// must be "tcp", "tcp4", or "tcp6". If
// laddr has a port of 0, ListenTCP will
// choose an available port. The caller can
// use the Addr method of TCPListener to
// retrieve the chosen address.
...
// Accept implements the Accept method in
// the Listener interface; it waits for the
// next call and returns a generic Conn.
...
// Addr returns the listener's network
// address, a *TCPAddr.
...
// Close stops listening on the TCP
...
// File returns a copy of the underlying
// os.File, set to blocking mode. It is the
// caller's responsibility to close f when
// finished. Closing l does not affect f,
// and closing f does not affect l.
//
// The returned os.File's file descriptor
// is different from the connection's.
...
// SetDeadline sets the deadline associated
// with the listener. A zero time value
// disables the deadline.
func (l *TCPListener) SetDeadline(t time.Time) error

// UDPAddr represents the address of a UDP end point.

// UDPAddr represents the address of a UDP
// end point.
type UDPAddr struct {
	IP   IP
	Port int
...
// ResolveUDPAddr parses addr as a UDP
// address of the form "host:port" or
// "[ipv6-host%zone]:port" and resolves a
// pair of domain name and port name on the
// network net, which must be "udp", "udp4"
// or "udp6". A literal address or host
// name for IPv6 must be enclosed in square
// brackets, as in "[::1]:80",
// "[ipv6-host]:http" or
// "[ipv6-host%zone]:80".
...
// UDPConn is the implementation of the
// Conn and PacketConn interfaces for UDP
// network connections.
type UDPConn struct {
	// contains filtered or unexported fields
}
...
// DialUDP connects to the remote address
// raddr on the network net, which must be
// "udp", "udp4", or "udp6". If laddr is
// not nil, it is used as the local address
// for the connection.
...
// ListenMulticastUDP listens for incoming
// multicast UDP packets addressed to the
// group address gaddr on ifi, which
// specifies the interface to join.
// ListenMulticastUDP uses default
...
// ListenUDP listens for incoming UDP
// packets addressed to the local address
// laddr. Net must be "udp", "udp4", or
// "udp6". If laddr has a port of 0,
// ListenUDP will choose an available port.
// The LocalAddr method of the returned
// UDPConn can be used to discover the
// port. The returned connection's ReadFrom
// and WriteTo methods can be used to
// receive and send UDP packets with
...
// ReadFromUDP reads a UDP packet from c,
// copying the payload into b. It returns
// the number of bytes copied into b and
// the return address that was on the
// packet.
//
// ReadFromUDP can be made to time out and
// return an error with Timeout() == true
// after a fixed time limit; see
// SetDeadline and SetReadDeadline.
...
// ReadMsgUDP reads a packet from c,
// copying the payload into b and the
// associated out-of-band data into oob. It
// returns the number of bytes copied into
// b, the number of bytes copied into oob,
// the flags that were set on the packet
...
// WriteMsgUDP writes a packet to addr via
// c, copying the payload from b and the
// associated out-of-band data from oob. It
// returns the number of payload and
// out-of-band bytes written.
...
// via c, copying the payload from b.
//
// WriteToUDP can be made to time out and
// return an error with Timeout() == true
// after a fixed time limit; see
// SetDeadline and SetWriteDeadline. On
// packet-oriented connections, write
...
// UnixAddr represents the address of a
...
// ResolveUnixAddr parses addr as a Unix
// domain socket address. The string net
// gives the network name, "unix",
...
// UnixConn is an implementation of the
// Conn interface for connections to Unix
// domain sockets.
...
// DialUnix connects to the remote address
// raddr on the network net, which must be
// "unix", "unixgram" or "unixpacket". If
// laddr is not nil, it is used as the
...
// datagram packets addressed to the local
// address laddr. The network net must be
// "unixgram". The returned connection's
// ReadFrom and WriteTo methods can be used
// to receive and send packets with
// per-packet addressing.
...
// CloseRead shuts down the reading side of
// the Unix domain connection. Most callers
// should just use Close.
...
// CloseWrite shuts down the writing side
// of the Unix domain connection. Most
// callers should just use Close.
...
// ReadFromUnix reads a packet from c,
// copying the payload into b. It returns
// the number of bytes copied into b and
// the source address of the packet.
//
// ReadFromUnix can be made to time out and
// return an error with Timeout() == true
// after a fixed time limit; see
// SetDeadline and SetReadDeadline.
...
// ReadMsgUnix reads a packet from c,
// copying the payload into b and the
// associated out-of-band data into oob. It
// returns the number of bytes copied into
// b, the number of bytes copied into oob,
...
// WriteMsgUnix writes a packet to addr via
// c, copying the payload from b and the
// associated out-of-band data from oob. It
// returns the number of payload and
// out-of-band bytes written.
...
// WriteToUnix writes a packet to addr via
// c, copying the payload from b.
//
// WriteToUnix can be made to time out and
// return an error with Timeout() == true
// after a fixed time limit; see
// SetDeadline and SetWriteDeadline. On
// packet-oriented connections, write
...
// UnixListener is a Unix domain socket
// listener. Clients should typically use
// variables of type Listener instead of
...
// ListenUnix announces on the Unix domain
// socket laddr and returns a Unix
// listener. The network net must be "unix"
// or "unixpacket".
...
// AcceptUnix accepts the next incoming
...
// Close stops listening on the Unix
// address. Already accepted connections
// are not closed.
