...
// 返回一个实现了CRAM-MD5身份认证机制（参见RFC
// 2195）的Auth接口。返回的接口使用给出的用户名和密码，采用响应——回答机制与服务端进行身份认证。
...
// 返回一个实现了PLAIN身份认证机制（参见RFC
// 4616）的Auth接口。返回的接口使用给出的用户名和密码，通过TLS连接到主机认证，采用identity为身份管理和行动（通常应设identity为""，以便使用username为身份）。
