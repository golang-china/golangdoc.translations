...
// 和基本的目标路径。如果目标路径是“/base”并且进入的请求的路径是“/dir”， 那
// 么最终请求的目标路径就会变成/base/dir。 NewSingleHostReverseProxy 不会重写
// Host header。要重写 Host header 请直接使用 ReverseProxy 自定的 Director 政
// 策。
...
// called before the user or Read have signaled the end of the keep-alive logic.
...
// Read returns the next request on the wire. An ErrPersistEOF is returned if it
// is gracefully determined that there are no more requests (e.g. after the
