...
//
// The http package has transparent support for the HTTP/2 protocol when using
// HTTPS. Programs that must disable HTTP/2 can do so by setting
// Transport.TLSNextProto (for clients) or Server.TLSNextProto (for servers) to
// a non-nil, empty map. Alternatively, the following GODEBUG environment
...
const (
	...
	// StateActive代表一个已经读取了请求数据1到多个字节的连接。 用于StateAlive的
	// Server.ConnState回调函数在将连接交付给处理器之前被触发， 等到请求被处理完
	// 后，Server.ConnState回调函数再次被触发。 在请求被处理后，连接状态改变为
	// StateClosed、StateHijacked或StateIdle。
	StateActive
...
var DefaultTransport RoundTripper = &Transport{Proxy: ProxyFromEnvironment, DialContext: (&net.Dialer{Timeout: 30 * time.Second, KeepAlive: 30 * time.Second}).DialContext, MaxIdleConns: 100, IdleConnTimeout: 90 * time.Second, TLSHandshakeTimeout: 10 * time.Second, ExpectContinueTimeout: 1 * time.Second}
...
// using Transport's CloseIdleConnections method and the MaxIdleConnsPerHost and
...
// URLs, depending on whether the server supports HTTP/2. See the package docs
// for more about HTTP/2.
...
type Transport struct {
	...
	// connections. If DialContext is nil (and the deprecated Dial below is also
	// nil), then the transport dials using package net.
	DialContext func(ctx context.Context, network, addr string) (net.Conn, error)

	// Dial specifies the dial function for creating unencrypted TCP
	// connections.
	//
	// Deprecated: Use DialContext instead, which allows the transport to cancel
	// dials as soon as they are no longer needed. If both are set, DialContext
	// takes priority.
...
// DetectContentType函数实现了 http://mimesniff.spec.whatwg.org/描述的算法，用于
// 确定数据的Content-Type。函数 总是返回一个合法的MIME类型；如果它不能确定数据的
// 类型，将返回 "application/octet-stream"。它最多检查数据的前512字节。
...
// PostFormValue returns the first value for the named component of the POST or
// PUT request body. URL query parameters are ignored. PostFormValue calls
// ParseMultipartForm and ParseForm if necessary and ignores any errors returned
// by these functions. If key is not present, PostFormValue returns the empty
// string.
