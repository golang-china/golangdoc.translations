...
// MinRead 是被 Buffer.ReadFrom 传递给 Read 调用的切片的最小尺寸. 只要 Buffer 在
// 保存 r 内容之外有最少 MinRead 字节的余量, 其 ReadFrom 方法就不会增加底层的缓
// 冲.
const MinRead = 512
...
// Reader 类型通过从一个 byte 切片读取数据, 实现了 io.Reader, io.Seeker,
// io.ReaderAt, io.WriterTo, io.ByteScanner, io.RuneScanner 接口.
type Reader struct {
}
...
// IndexRune 返回 unicode 字符 r 的 utf-8 编码在 s 中第一次出现的位置, 不存在则
// 返回 -1.
...
// Join 将一系列 byte 切片连接为一个 byte 切片, 之间用 sep 来分隔, 返回生成的新
// 切片.
...
// Map 将 s 的每一个 unicode 码值 r 都替换为 mapping(r), 返回这些新码值组成的切
// 片拷贝. 如果 mapping 返回一个负值, 将会丢弃该码值而不会被替换.
...
// NewBuffer 使用 buf 作为初始内容创建并初始化一个 Buffer. 本函数用于创建一个用
// 于读取已存在数据的 buffer, 也用于指定用于写入的内部缓冲的大小. 此时, buf 应为
// 一个具有指定容量但长度为 0 的切片. buf会 被作为返回值的底层缓冲切片.
//
...
// ToLowerSpecial 使用 _case 规定的字符映射, 返回将所有字母都转为对应的小写版本
// 的拷贝.
...
// ToTitleSpecial 使用 _case 规定的字符映射, 返回将所有字母都转为对应的标题版本
// 的拷贝.
...
// ToUpperSpecial 使用 _case 规定的字符映射, 返回将所有字母都转为对应的大写版本
// 的拷贝.
...
// ReadBytes 读取直到第一次遇到 delim 字节, 返回一个包含已读取的数据和delim字节
...
// ReadString 读取直到第一次遇到 delim 字节, 返回一个包含已读取的数据和delim字节
...
// WriteTo 从缓冲中读取数据直到缓冲内没有数据或遇到错误, 并将这些数据写入 w. 返
