...
// Plan9是一个256色的调色板，将24位的RGB色彩立方划分为4x4x4的子立方，每个子立方
// 体都有4个色调。比起WebSafe，本方案通过将颜色立方划分成更少的方格减少了颜色的
// 分辨率，以便用额外的空间换取增加强度的分辨率。结果是灰色有16个色调（4个灰色子
// 立方每个有4个采样），初级和次级颜色各有13个色调（3个子立方每个有4个采样，再加
// 上黑色），加上对色彩立方其余部分的合理色彩选择，实现了更好的连续色调的表现。
//
// 本调色板用于Plan9操作系统，参见
// http://plan9.bell-labs.com/magic/man2html/6/color
...
// WebSafe是一个216色的调色板，被早期版本的Netscape Navigator（一种浏览器）广泛
// 使用，它也被称为Netcape色彩立方。细节参见
