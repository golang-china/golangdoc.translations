...
// ComplexType 在此只用作文档目的。 它代表所有的复数类型：即 complex64 或
// complex128。
...
// int 是带符号整数类型，其大小至少为32位。 它是一种确切的类型，而不是 int32 的
// 别名。
type int int
...
// int64 是所有带符号64位整数的集合。 范围：-9223372036854775808 至
// 9223372036854775807。
type int64 int64
...
// uint 是无符号整数类型，其大小至少为32位。 它是一种确切的类型，而不是 uint32
// 的别名。
type uint uint
...
// delete 内建函数按照指定的键将元素从映射中删除。 若 m 为 nil 或无此元素，
// delete 即为空操作。
...
// 任何被推迟的函数。这会持续到该Go程 中所有函数都按相反的顺序停止执行之后。此时
// ，该程序会被终止，而错误情况会被报告， 包括引发该恐慌的实参值。此终止序列称为
// 恐慌过程，并可通过内建函数 recover 控制。
