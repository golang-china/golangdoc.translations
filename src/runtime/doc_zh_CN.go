...
// Caller 报告关于调用Go程的栈上的函数调用的文件和行号信息。 实参 skip 为占用的
// 栈帧数，若为0则表示 Caller 的调用者。（由于历史原因，skip 的意思在 Caller 和
// Callers 中并不相同。）返回值报告程序计数器， 文件名及对应调用的文件中的行号。
// 若无法获得信息，布尔值 ok 即为 false。
...
// normally return the file and line number of the instruction immediately
// following the call. To easily look up file/line information for the call
// sequence, use Frames.
...
// len(p) >= n, MemProfile copies the profile into p and returns n, true. If
...
// sweeping, the profile only accounts for allocations that have had a chance to
...
// symbolic information. These are used when printing stack traces for a program
// that uses cgo.
//
...
//
// If the Context field is not 0, then it is a value returned by a previous call
// to the context function. This case is called when the context is no longer
// needed; that is, when the Go code is returning to its C code caller. This
// permits the context function to release any associated resources.
//
...
// function, in a typical program the context function will be called many times
// without ever recording a traceback for that context. Recording a complete
// stack trace in a call to the context function is likely to be inefficient.
//
...
// called from an execution thread other than the one that recorded the context,
// but only when the context is known to be valid and unchanging. The traceback
// function may also be called deeper in the call stack on the same thread that
// recorded the context. The traceback function may be called multiple times
// with the same Context value; it will usually be appropriate to cache the
// result, if possible, the first time this is called for a specific context
// value.
//
...
// (a C ucontext_t* cast to uintptr_t). This may be used to start tracing at the
// point where the signal occurred. If the traceback function is not called from
// a signal handler, SigContext will be zero.
//
...
// possible, setting them to 0/nil if the information is not available. The Data
// field may be used to store any useful information across calls. The More
// field should be set to non-zero if there is more information for this PC,
// zero otherwise. If More is set non-zero, the function will be called again
// with the same PC, and may return different information (this is intended for
// use with inlined functions). If More is zero, the function will be called
// with the next PC value in the traceback. When the traceback is complete, the
// function will be called once more with PC set to zero; this may be used to
// free any information. Each call will leave the fields of the struct set to
// the same values they had upon return, except for the PC field when the More
// field is zero. The function must not keep a copy of the struct pointer
// between calls.
//
...
// traceback function will be displayed as numbers. If the traceback function is
// nil, the symbolizer function will never be called. The context function may
// be nil, in which case the traceback function will only be called with the
// context field set to zero. If the context function is nil, then calls from Go
// to C to Go will not show a traceback for the C portion of the call stack.
//
...
// associated finalizer. Assuming that SetFinalizer is not called again, the
// next time the garbage collector sees that obj is unreachable, it will free
// obj.
//
// SetFinalizer(obj, nil) clears any finalizer associated with obj.
//
// The argument obj must be a pointer to an object allocated by calling new or
// by taking the address of a composite literal. The argument finalizer must be
// a function that takes a single argument to which obj's type can be assigned,
// and can have arbitrary ignored return values. If either of these is not true,
...
// includes a block with a finalizer, that cycle is not guaranteed to be garbage
// collected and the finalizer is not guaranteed to run, because there is no
...
// resources associated with an object during a long-running program. For
// example, an os.File object could use a finalizer to close the associated
// operating system file descriptor when a program discards an os.File without
// calling Close, but it would be a mistake to depend on a finalizer to flush an
// in-memory I/O buffer such as a bufio.Writer, because the buffer would not be
...
// finalizers correctly, the program must ensure that the object is reachable
// until it is no longer required. Objects stored in global variables, or that
// can be found by tracing pointers from a global variable, are reachable. For
// other objects, pass the object to a call of the KeepAlive function to mark
// the last point in the function where the object must be reachable.
//
// For example, if p points to a struct that contains a file descriptor d, and p
// has a finalizer that closes that file descriptor, and if the last use of p in
// a function is a call to syscall.Write(p.d, buf, size), then p may be
// unreachable as soon as the program enters syscall.Write. The finalizer may
// run at that moment, closing p.d, causing syscall.Write to fail because it is
...
// tracing is already enabled. Most clients should use the runtime/trace package
// or the testing package's -test.trace flag instead of calling StartTrace
// directly.
...
// Version 返回Go目录树的版本字符串。 它一般是一个提交散列值及其构建时间，也可能
// 是一个类似于 "go1.3" 的发行标注。
func Version() string
