...
// 时更改此百分比。 详见 http://zh.golanger.com/pkg/runtime/debug/#SetGCPercent
// 。
//
...
// skip 为开始在 pc 中记录之前所要跳过的栈帧数，若为 0 则表示 Callers 自身的栈帧
// ， 若为 1 则表示 Callers 的调用者。它返回写入到 pc 中的项数。
//
// 注意，由于每个切片项 pc[i] 都是一个返回程序计数器，因此查找 pc[i] 的文件和行
// （例如，使用 (*Func).FileLine）将会在该调用之后立即返回该指令所在的文件和行号
// 。 要查找该调用本身所在的文件和行号，请使用 pc[i]-1。此规则的一个例外是，若
// pc[i-1] 对应于函数 runtime.sigpanic，那么 pc[i] 就是失败指令的程序计数器，因
// 此应当不通过任何减法来使用。
...
// x 再次变得可访问，但现在没有了相关联的终结器。 假设 SetFinalizer 未被再次调用
// ，当下一次垃圾回收器发现 x 无法访问时，就会释放 x。
//
