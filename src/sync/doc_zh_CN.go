...
// an arbitrary number of readers or a single writer. RWMutexes can be created
// as part of other structures; the zero value for a RWMutex is an unlocked
// mutex.
//
...
// lock is released. In particular, this prohibits recursive read locking. This
// is to ensure that the lock eventually becomes available; a blocked Lock call
...
// Wait 原子性地解锁 c.L 并挂起调用的Go程的执行。不像其它的系统那样，Wait 不会返
// 回，除非它被 Broadcast 或 Signal 唤醒。
//
// 由于 Wait 第一次恢复时 c.L 并未锁定，因此调用者一般不能假定 Wait 返回时条件为
// 真。 取而代之，调用者应当把 Wait 放入循环中：
//
// 	c.L.Lock()
// 	for !condition() {
// 	    c.Wait()
...
// 	config.once.Do(func() { config.init(filename) })
//
// 由于 f 的调用返回之前没有 Do 的调用会返回，因此若 f 引起了 Do 的调用，它就会
// 死锁。
//
// 若 f 发生派错（panic），Do 会考虑它是否已返回；将来对 Do 的调用会直接返回而不
// 调用 f。
...
// 么它可以在任何时候发生。 一般来说，这意味着对 Add 的调用应当在该语句创建Go
// 程，或等待其它事件之前执行。 具体见 WaitGroup 的示例。
