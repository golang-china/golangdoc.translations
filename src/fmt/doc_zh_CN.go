...
// example, given 12.345 the format %6.3f prints 12.345 while %.3g prints 12.3.
// The default precision for %e and %f is 6; for %g it is the smallest number of
...
// considerations apply for operands that implement certain interfaces. In order
// of application:
//
...
// will quote each element of a slice of strings, and %6.2f will control
// formatting for each element of a floating-point array.
//
// However, when printing a byte slice with a string-like verb (%s %q %x %X), it
// is treated identically to a string, as a single item.
//
...
// the fmt package reformats the error message from the panic, decorating it
// with an indication that it came through the fmt package. For example, if a
// String method calls panic("bad"), the resulting formatted message will look
// like
//
// 	%!s(PANIC=bad)
//
...
// differs from the corresponding routines in C, which uniformly treat newlines
// as spaces.
//
...
// format and the input. With that proviso, text in the format string must match
// the input text; scanning stops if it does not, with the return value of the
// function indicating the number of arguments scanned.
//
...
// with a precision (no %5.2f, just %5f). If width is provided, it applies after
// leading spaces are trimmed and specifies the maximum number of runes to read
...
// it implements the Scanner interface) that method will be used to scan the
// text for that operand. Also, if the number of arguments scanned is less than
// the number of arguments provided, an error is returned.
//
...
// To attach ReadRune and UnreadRune methods to a reader without that
...
//
// 宽度与精度的控制格式以Unicode码点为单位。（这点与C的 printf 不同， 它以字节数
// 为单位。）二者或其中之一均可用字符 '*' 表示， 此时它们的值会从下一个操作数中
// 获取，该操作数的类型必须为 int。
//
// 对数值而言，宽度为该数值占用区域的最小宽度；精度为小数点之后的位数。 但对于
// %g/%G 而言，精度为所有数字的总数。例如，对于123.45，格式 %6.2f 会打印123.45，
// 而 %.4g 会打印123.5。%e 和 %f 的默认精度为6；但对于 %g 而言， 它的默认精度为
// 确定该值所必须的最小位数。
//
// 对大多数值而言，宽度为输出的最小字符数，如果必要的话会为已格式化的形式填充空
// 格。 对字符串而言，精度为输出的最大字符数，如果必要的话会直接截断。
//
// 其它标记：
//
...
// %X），以下两条规则也适用：
//
// 1. 若一个操作数实现了 error 接口，Error 方法就能将该对象转换为字符串， 随后会
// 根据占位符的需要进行格式化。
//
// 2. 若一个操作数实现了 String() string 方法，该方法能将该对象转换为字符串， 随
// 后会根据占位符的需要进行格式化。
//
// 为避免以下这类递归的情况：
//
...
// 格式化错误：
//
// 如果给占位符提供了无效的实参（例如将一个字符串提供给 %d）， 所生成的字符串会
// 包含该问题的描述，如下例所示：
//
...
// os.Stdin 中读取；Fscan、Fscanf 和 Fscanln 从指定的 io.Reader 中读取； Sscan、
// Sscanf 和 Sscanln 从实参字符串中读取。Scanln、Fscanln 和 Sscanln 在换行符处停
// 止扫描，且需要条目紧随换行符之后；Scanf、Fscanf 和 Sscanf 需要输入换行符来匹
// 配格式中的换行符；其它函数则将换行符视为空格。
//
// Scanf、Fscanf 和 Sscanf 根据格式字符串解析实参，类似于 Printf。例如，%x 会将
// 一个整数扫描为十六进制数，而 %v 则会扫描该值的默认表现格式。
//
...
//
// 当以某种格式进行扫描时，无论在格式中还是在输入中，所有非空的连续空白字符 （除
// 换行符外）都等价于单个空格。由于这种限制，格式字符串文本必须匹配输入的文本，
// 如果不匹配，扫描过程就会停止，并返回已扫描的实参数。
//
// 在所有的扫描参数中，若一个操作数实现了 Scan 方法（即它实现了 Scanner 接口），
// 该操作数将使用该方法扫描其文本。此外，若已扫描的实参数少于所提供的实参数， 就
// 会返回一个错误。
//
...
type ScanState interface {
	...
	// ReadRune reads the next rune (Unicode code point) from the input. If
	// invoked during Scanln, Fscanln, or Sscanln, ReadRune() will return EOF
	// after returning the first '\n' or when reading beyond the specified
	// width.
	//
	...
	// Token skips space in the input if skipSpace is true, then returns the run
	// of Unicode code points c satisfying f(c). If f is nil,
	// !unicode.IsSpace(c) is used; that is, the token will hold non-space
	// characters. Newlines are treated appropriately for the operation being
	// performed; see the package documentation for more information. The
	// returned slice points to shared data that may be overwritten by the next
	// call to Token, a call to a Scan function using the ScanState as input, or
	// when the calling Scan method returns.
	...
	// 的Unicode码点 c。若 f 为 nil，就会使用 !unicode.IsSpace(c)； 即，该标记会
	// 保留非空格字符。换行符会被视作空格，除非该扫描操作为 Scanln、 Fscanln 或
	// Sscanln，否则在这种情况下，换行符会被视作 EOF。 所返回的指向共享数据的切
	// 片可能会在下次调用 Token 时被覆盖，Scan 函数的调用 会将 ScanState 用作输
	// 入，或当调用 Scan 方法返回时也会。
	...
	// Because ReadRune is implemented by the interface, Read should never be
	// called by the scanning routines and a valid implementation of ScanState
	// may choose always to return an error from Read.
	//
...
type State interface {
	...
	// Precision returns the value of the precision option and whether it has
	// been set.
...
// Fscan 扫描从 r 中读取的文本，并将连续由空格分隔的值存储为连续的实参。 换行符
// 计为空格。它返回成功扫描的条目数。若它少于实参数，err 就会报告原因。
...
// Scan 扫描从标准输入中读取的文本，并将连续由空格分隔的值存储为连续的实参。 换
// 行符计为空格。它返回成功扫描的条目数。若它少于实参数，err 就会报告原因。
...
// 格式由 format 决定。它返回成功扫描的条目数。若返回的条目数小于实参数， 则会报
// 告错误原因 err。输入中的换行符必须与 format 中的相匹配，除了一种例外： %c 占
// 位符总是扫描输入中的下一个符文，即便它是空格（或 tab 等）或换行符。
...
// Sscan 扫描实参 string，并将连续由空格分隔的值存储为连续的实参。 换行符计为空
// 格。它返回成功扫描的条目数。若它少于实参数，err 就会报告原因。
