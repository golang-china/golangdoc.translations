...
// lines and other line comments and with a blank line following the comment, to
// separate it from the package documentation. Unlike build constraints, this
...
type Context struct {
	...
	// Go releases the current release is compatible with. In addition to the
	// BuildTags and ReleaseTags, build constraints consider the values of
	// GOARCH and GOOS as satisfied tags.
	BuildTags   []string
	ReleaseTags []string

	// The install suffix specifies a suffix to use in the name of the
	// installation directory. By default it is empty, but custom builds that
	// need to keep their outputs separate can set InstallSuffix to do so. For
	// example, when using the race detector, the go command uses InstallSuffix
	// = "race", so that on a Linux/386 system, packages are written to a
	// directory named "linux_386_race" instead of the usual "linux_386".
	...
	// IsDir reports whether the path names a directory. If IsDir is nil, Import
	// calls os.Stat and uses the result's IsDir method.
...
// object file suffix, and the default linker output name. As of Go 1.5, those
