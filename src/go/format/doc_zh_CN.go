...
// ast.Stmt. Node does not modify node. Imports are not sorted for nodes
// representing partial source files (i.e., if the node is not an *ast.File or a
// *printer.CommentedNode not wrapping an *ast.File).
//
...
// Source formats src in canonical gofmt style and returns the result or an (I/O
// or syntax) error. src is expected to be a syntactically correct Go source
// file, or a list of Go declarations or statements.
//
