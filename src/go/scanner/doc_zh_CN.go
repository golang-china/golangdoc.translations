...
// error and err is not nil. Also, for each error encountered, the Scanner field
// ErrorCount is incremented by one. The mode parameter determines how comments
// are handled.
//
...
//
// For more tolerant parsing, Scan will return a valid token if possible even if
// a syntax error was encountered. Thus, even if the resulting token sequence
