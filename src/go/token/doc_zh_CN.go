...
// 一套表达式分析优先级常量. 非运算符的优先级最低, 其后是优先级为 1 开始的运算
// 符, 一直到一元运算符. 优先级最高的用于 "catch-all", 总是优先于选择器, 索引以
// 及其它运算符和定界符.
const (
...
// respective file to the current file set (via FileSet.AddFile) and then call
...
// current value of the FileSet's Base() is used instead.
//
...
// AddFile 新增一个文件到文件集, 给定该文件的文件名, 基本偏移量, 和文件尺寸, 并
// 返回该文件. 多个文件可以同名. 基本偏移量不得小于文件集的 Base(), 也不能为负.
// 典型的, 如果提供了负数基本偏移量, 使用当前文件集的 Base() 替代.
//
// 增加文件会设置文件集的 Base() 为 base + size + 1, 作为下一个文件的最小 base
// 值. 在 Pos 值 p 和给定文件偏移量直接存在下述关系:
//
// 	int(p) = base + offs
//
// offs 在 [0, size] 范围内, 因而 p 在 [base, base+size] 范围内. 为了方便, 可用
// File.Pos 从一个文件偏移量创建文件专用的位置值.
...
// comments; otherwise those comments are ignored. p must be a Pos value in s or
// NoPos.
