...
// A FlagSet creates and parses command-line flags. It is similar to the
...
// A Symbolizer annotates a profile with symbol information. The profile was
// fetch from src. The meaning of mode is defined by the implementation.
...
type UI interface {
	...
	// Print shows a message to the user. It formats the text as fmt.Print would
	// and adds a final \n if not already present. For line-based UI, Print
	// writes to standard error. (Standard output is reserved for report data.)
	Print(...interface{})

	// PrintErr shows an error message to the user. It formats the text as
	// fmt.Print would and adds a final \n if not already present. For
	// line-based UI, PrintErr writes to standard error.
	PrintErr(...interface{})
	...
	// SetAutoComplete instructs the UI to call complete(cmd) to obtain the
	// auto-completion of cmd, if the UI supports auto-completion at all.
...
// NoObjTool returns a trivial implementation of the ObjTool interface. Open
