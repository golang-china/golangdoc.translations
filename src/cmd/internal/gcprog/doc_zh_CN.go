...
// Program Format
//
// The GC program encodes a sequence of 0 and 1 bits indicating scalar or pointer
// words in an object. The encoding is a simple Lempel-Ziv program, with codes to
// emit literal bits and to repeat the last n bits c times.
//
...
//
// The numbers n and c, when they follow a code, are encoded as varints using the
// same encoding as encoding/binary's Uvarint.
...
// that the index'th word in the object memory is a pointer. Any bits between the
// current index and the new index are set to zero, meaning the corresponding words
// are scalars.
func (*Writer) Ptr(index int64)

// Repeat emits an instruction to repeat the description of the last n words c times
// (including the initial description, c+1 times in total).
...
// ZeroUntil adds zeros to the bit stream until reaching the given index; that is,
// it records that the words from the most recent pointer until the index'th word
// are scalars. ZeroUntil is usually called in preparation for a call to Repeat,
// Append, or End.
